...
\begin{definition}
Кажуть, що ненулеве число $a \in \mathbb{Z}$ \textbf{ділить число} $b \in \mathbb{Z}$, якщо
\begin{align*}
\exists c \in \mathbb{Z}: b = ac.
\end{align*}
Позначення: $a \mid b$.\\
Інколи кажуть, що $b$ \textbf{ділиться націло на} $a$, та позначають за $b \, \vdots \, a$.
\end{definition}
...
\begin{remark}
$a \mid 0$, де $a$ -- будь-яке ненульове число.
\end{remark}

\begin{remark}
Кожне число $b \neq 0$ має скінченну кількість дільників.\\
Дійсно, візьмімо якийсь дільник $a$ числа $b$, тобто $a \mid b$, то звідси $b = ac$, а тому 
\[
|b| = |a||c| \implies |b| \geq |a|.
\]
Остання нерівність підтверджує слова.
\end{remark}
...
\item $b \mid a \implies a = bx$. $c \mid b \implies b = yc \implies a = bx = xy \cdot c \implies c \mid a$. \qedhere
\end{enumerate}
\end{proof}

\begin{example}
Будь-який добуток двох сусідніх чисел ділиться на $2$.\\
Нехай $a \in \mathbb{Z}$, розглянемо добуток $a(a+1)$. Маємо кілька випадків:
\begin{itemize}[nosep,wide=0pt]
\item якщо $a = 2k, k \in \mathbb{Z}$, то отримаємо $2 \mid 2k(2k+1)$;
\item якщо $a = 2k+1, k \in \mathbb{Z}$, то отримаємо $2 \mid (2k+1)(2k+2)$.
\end{itemize}
Звідси випливає, що $a(a+1) = 2x$, тобто $2 \mid a(a+1)$.
\end{example}

\begin{example}
Довести, що різниця послідовних кубів ніколи не ділиться на $2$.
\begin{align*}
(a+1)^3 - a^3 = 3a^2+3a+1 = 3a(a+1)+1.
\end{align*}
Зауважимо, що $2 \mid a(a+1)$, ми щойно доводили. Тобто права частина -- непарне число, тобто $2 \nmid (a+1)^3 - a^3$.
\end{example}

\begin{example}
Довести, що кількість дільників квадрата натурального числа -- непарна.\\
Розглянемо число $n^2$. Нехай $d \mid n^2$, тоді звідси $\dfrac{n^2}{d} \mid n^2$. Отже, маючи дільник $d$, ми отримуємо інший дільник $\dfrac{n^2}{d}$ -- отримуємо парне число дільників. Плюс, ще $n \mid n^2$, тому загалом буде непарне число.
\bigskip \\
До речі, навпаки теж працює. Нехай $m$ -- число, яке містить непарне число дільників. Тоді $m$ -- квадрат деякого числа.
\end{example}

\begin{lemma}[Ділення з остачею]
Для довільних $a,b \in \mathbb{Z}$, де число $a > 0$, існують єдині $q,r \in \mathbb{Z}$, для яких \[
b = qa + r
\]
де $r$ задовольняє нерівність $0 \leq r < a$. Число $r$ називають \textbf{остачею} від ділення $b$ числом $a$.
\end{lemma}

\begin{proof}
I. \textit{Існування}.\\
Розглянемо множину \[
S = \{b - qa : q \in \mathbb{Z}\} \cap \mathbb{Z}_{\geq 0}. 
\]
Ясно, що вона непорожня, оскільки там лежить число в залежності від числа $b$:
\begin{itemize}[nosep,wide=0pt]
\item або $b \in S$ при $b \geq 0$, тому що $b = b - 0 \cdot a$;
\item або $b - ba \in S$ при $b < 0$, тому що $b-b \cdot a = b(1-a)$.
\end{itemize}
Також ця множина обмежена знизу, оскільки кожне число -- невід'ємне. Значить, існує найменше число $\min S$. Цей мінімум позначимо за число $r$, яке шукали.\\
...
\[
r - a = b-qa - a = b - (q+1)a \overset{q+1 = q^*}{=} b-q^* a.
\]
...
\]
Із цієї рівності випливає, що $a \mid (r-r')$. Але ми водночас маємо, що $0 < r - r' < r < a$, тоді єдиний варіант під час ділення -- це випадок $r-r' = 0 \implies r = r'$. Тоді $q' = q$. Суперечність!
\end{proof}
...
Для даної леми існують певні модифікації на остачі, які ще знадобляться пізніше, тож я їх сформулюю.

\begin{corollary}
Для довільних $a,b \in \mathbb{Z}$, де число $a \neq 0$, існують єдині $q,r \in \mathbb{Z}$, для яких \[
b = qa+r,
\] де число $r$ задовольняє нерівність $0 \leq r < |a|$.\\
...
b = qa+r,
\]
де число $r$ задовольняє нерівність $-\dfrac{1}{2}|a| < r \leq \dfrac{1}{2}|a|$.
\end{corollary}

\begin{proof}
Маємо $b = q'a+r'$, де $0 \leq r' < |a|$. Остача має кілька можливих сценарїїв:
\begin{itemize}[nosep,wide=0pt]
\item $0 \leq r' < \dfrac{|a|}{2}$, то тоді $r = r'$ та $q = q'$;
\item $\dfrac{|a|}{2} \leq r' < |a|$, то тоді $r = r' - |a|$ та $q = q'+\text{sgn } a$.
\end{itemize}
У обох випадках ми знайшли $r,q$ єдиним чином, для яких $b = qa+r$. Причому $\dfrac{-|a|}{2} < r \leq \dfrac{|a|}{2}$.
\end{proof}

\subsection{Найбільший спільний дільник}
\begin{definition}
\textbf{Найбільшим спільним дільником} чисел $a,b \in \mathbb{Z} \setminus \{0\}$ назвемо таке число:
\begin{align*}
\gcd(a,b) = \max\{c \in \mathbb{N}: c \mid a, c \mid b\}.
\end{align*}
Альтернативні позначення: $\text{НСД}(a,b)$ або $(a,b)$. Останнє позначення частіше можна зустріти в сучасних книгах.
\end{definition}
...
\begin{definition}
Числа $a,b \in \mathbb{Z} \setminus \{0\}$ називаються \textbf{взаємно простими}, якщо
\begin{align*}
\gcd(a,b) = 1.
\end{align*}
\end{definition}
...
\begin{example}
Зокрема $\gcd(49,35,28,2023) = 7$. Також $\gcd(11,13,21) = 1$, тобто вони взаємно прості.
\end{example}

\begin{proposition}
\label{gcd(a,b) = xa+yb}
Задані числа $a,b \in \mathbb{Z} \setminus \{0\}$. Тоді існують $x,y \in \mathbb{Z}$, для яких
\begin{equation}
\label{eq:gcd(a,b)_as_linear_combination}
\gcd(a,b) = x \cdot a + y \cdot b.
\end{equation}
Тобто НСД чисел $a,b$ можна розкласти як лінійну комбінацію чисел $a$ та $b$.
\end{proposition}

\begin{proof}
Розглянемо множину \[
S = \{xa+yb: x,y \in \mathbb{Z}\} \cap \mathbb{N}.
\]
Вона непорожня, однак який елемент там існує точно, залежить від ситуації:
\begin{itemize}[nosep,wide=0pt]
\item при $a > 0, b > 0$ елемент $a \cdot a + b \cdot b \in S$; (тобто взяли $x = a,\ y = b$);
\item при $a < 0, b > 0$ елемент $(-a) \cdot a + b \cdot b \in S$; (тобто взяли $x = -a,\ y = b$);
\item при $a > 0, b < 0$ елемент $a \cdot a + (-b) \cdot b \in S$; (тобто взяли $x = a,\ y = -b$);
\item при $a < 0, b < 0$ елемент $(-a) \cdot a + (-b) \cdot b \in S$; (тобто взяли $x = -a,\ y = -b$).
\end{itemize}
Також множина $S$ обмежена знизу, оскільки всі числа -- додатні. Тому існує найменше число $\min S$, цей мінімум позначимо за $c$. Для цього числа існують $x_0,y_0 \in \mathbb{Z}$, для яких $c = ax_0 + by_0$. А тепер покажемо, що $c = \gcd(a,b)$.\\
Поділимо $a$ на $c$ з остачею. Маємо $a = cq + r$, де число $0 \leq r < c$. Водночас $c = x_0a+y_0b$. Тоді
\[
...
Поділимо $b$  на $c$ з остачею. Абсолютно аналогічно доводиться, що $b \mid c$.\\
...
\begin{proof}
За щойно доведеним твердженням $\gcd(a,b) = xa + yb$ для $x,y \in \mathbb{Z}$. Маємо
\[
d \mid a,\ d \mid b \implies d \mid xa + yb = \gcd(a,b). \qedhere
\]
\end{proof}

\begin{corollary}
Задані числа $a,b \in \mathbb{Z} \setminus \{0\}$ та число $d = \gcd(a,b)$.
Тоді $\gcd(a_1,b_1) = 1$, де числа $a_1,b_1$ взялись від того факту, що $a = da_1$ та $b = db_1$ -- означення подільності. \\
Простіше кажучи, $\gcd\left( \dfrac{a}{d}, \dfrac{b}{d} \right) = 1$.
\end{corollary}

\begin{proof}
За \prpref{gcd(a,b) = xa+yb} маємо $\gcd (a_1,b_1) = a_1 x + b_1 y$. Помножимо на $d$ -- одержимо:
\[
...
\begin{corollary}
Маємо $\gcd(a,b) = 1$ та $\gcd(a,c) = 1$. Тоді $\gcd(a,bc) = 1$.
\end{corollary}
...
\]
Із цієї рівності випливає, що $d \mid c$, але тоді звідси $d$ -- спільний дільник чисел $a,c$. Тож одержимо $d \mid \gcd(a,c) = 1 \implies d = 1$. Суперечність!\\
...
Маємо $d_2 \mid b, d_2 \mid b-a$. Тоді $d_2 \mid a = b-(b-a)$. Отже, $d_2$ -- спільний дільник чисел $a,b$. Тож $d_2 \mid d_1$.\\
...
Справедлива рівність:
\begin{align*}
\gcd(a,b) = \gcd(b, b-xa), \qquad x \in \mathbb{Z}.
\end{align*}
\end{corollary}

\begin{theorem}[Алгоритм Евкліда]
Задані числа $a,b \in \mathbb{N}$, де $b > a$. Послідовно використаємо ділення за остачею таким чином:
\begin{center}
\begin{tabular}{ll}
$b = aq_1 + r_1$ & $0 < r_1 < a$ \\
...
\gcd(b,a) \overset{\crlref{gcd(a,b) = gcd(b,b-xa)}}{=} \gcd(a,b-aq_1) = \gcd(a,r_1).
\]
Позначимо $a = r_0$. Тоді отримаємо $\gcd(b,a) = \gcd(r_0,r_1)$.\\
Далі припустимо, що $\gcd(a,b) = \gcd(r_n,r_{n+1})$. Доведемо звідси, що $\gcd(a,b) = \gcd(r_{n+1},r_{n+2})$. Дійсно, \[
\gcd(r_{n},r_{n+1}) \overset{\crlref{gcd(a,b) = gcd(b,b-xa)}}{=} \gcd(r_{n+1}, r_n - r_{n+1}q_n) = \gcd(r_{n+1}, r_{n+2}).
\]
Тобто за МІ $\gcd(b,a) = \gcd(r_{n},r_{n+1})$. А тому звідси випливає, що $\gcd(b,a) = \gcd(r_k,r_{k-1}) = r_k$.
\end{proof}

\begin{example}
\label{gcd using Euclidean}
Знайти $\gcd(392, 693)$.\\
Скористаємось алгоритмом Евкліда -- одержимо такі рівності:
\begin{gather*}
693 =´392 \cdot 1 + 301; \\
392 = 301 \cdot 1 + 91; \\
301 = 91 \cdot 3 + 28; \\
91 = 28 \cdot 3 + \textcolor{red}{7}; \\
28 = 7 \cdot 4.
\end{gather*}
Число $\textcolor{red}{7}$ -- остання ненульова остача. Тоді $\gcd(392, 693) = 7$.
\end{example}
...
\end{gather*}
Таким чином, $\gcd(392, 693) = 23 \cdot 392 - 13 \cdot 693$.
...
ax + by = c,
\end{equation*}
де $x,y \in \mathbb{Z}$ -- невідомі; $a,b,c \in \mathbb{Z} \setminus \{0\}$. Мета: знайти роз'язок в цілих числах.
\bigskip \\
I. \textit{$\gcd(a,b) \nmid c$.}\\
У такому разі розв'язків нема. Поступимо від супротивного.\\
!Припустімо, що $x_0,y_0 \in \mathbb{Z}$ -- деякий розв'язок рівняння $ax+by=c$. Відомо, що $\gcd(a,b) \mid a$ та $\gcd(a,b) \mid b$, а тому звідси $\gcd(a,b) \mid ax_0 + by_0 = c$. Суперечність!
\bigskip \\
II. \textit{$\gcd(a,b) \mid c$.}\\
Оскільки всі числа $a,b,c$ діляться націло на $\gcd(a,b)$, то можна обидві частини поділити на це число -- отримаємо
\[
a_1x + b_1y = c_1,
\]
причому тут $\gcd(a_1,b_1) = 1$. Однак $\gcd(a_1,b_1) = a_1 x + b_1 y = 1$ для деяких $x,y \in \mathbb{Z}$. Помножимо на число $c_1$ -- одержимо:
\[
a_1 (c_1x) + b_1 (c_1y) = c_1.
\]
А потім ще помножимо на $\gcd (a,b)$ -- отримаємо:
\[
a_1 x_0 + b_1 y_0 = c.
\]
...
\]
Поділимо на $\gcd(a,b)$, тоді матимемо наступне:
\[
a_1 (x_1-x_0) + b_1(y_1-y_0) = 0 \implies a_1(x_1-x_0) = b_1(y_0-y_1).
\]
Числа $a_1,b_1$ -- взаємно прості. Тому для рівності треба вимагати, щоб $b_1 \mid (x_1-x_0)$. Звідси матимео $x_1 - x_0 = mb_1$ для $m \in \mathbb{Z}$. Тоді звідси $y_0 - y_1 = ma_1$. Тобто
\[
\begin{cases} x_1 = x_0 + mb_1 \\ y_1 = y_0 - ma_1 \end{cases},
\] де $m \in \mathbb{Z}$ -- ще один розв'язок.
\bigskip \\
Ми щойно одержили такий результат.
\begin{theorem}
Нехай $a,b,c \in \mathbb{Z}$. Лінійне діофантове рівняння
\begin{equation}
ax + by = c
\end{equation}
має розв'язок $\iff \gcd(a,b) \mid c$.\\ Якщо $(x_0,y_0)$ -- деякий розв'язок, то\\
\[ \begin{cases} x_1 = x_0 + mb_1, \\ y_1 = y_0 - ma_1, \end{cases} m \in \mathbb{Z}, \]
задають інші розв'язки, де $b_1 = \dfrac{b}{\gcd(a,b)},\ a_1 = \dfrac{a}{\gcd(a,b)}$.
\end{theorem}

Єдине питання полягає в тому, як на практичному рівні знайти перший розв'язок $(x_0,y_0)$, щоб врешті-решт знайти інші. Для цього треба скористатися алгоритмом Евкліда.

\begin{example}
Розв'язати рівняння $392x + 693y = 14$.\\
У прикладі \exref{gcd using Euclidean} ми обчислили $\gcd(392,693) = 7 \mid 14$. Тобто рівняння розв'язок точно має. Але з прикладу \exref{gcd as linear combination using Euclidean}, ми отримали розклад $\gcd(392, 693)$ на лінійну комбінацію таким чином:
\[
...
\begin{definition}
\textbf{Найменшим спільним кратним} чисел $a,b \in \mathbb{Z} \setminus \{0\}$ назвемо таке число:
\begin{align*}
...
\begin{theorem}
Задані $a,b \in \mathbb{N}$. Тоді
\begin{equation}
\label{eq:lcm_times_gcd}
\gcd(a,b) \cdot \lcm(a,b) = ab.
\end{equation}
\end{theorem}

\begin{proof}
Позначимо $d = \gcd(a,b)$. Звідси $a=dr$ та $b=ds$ для деяких $r,s \in \mathbb{N}$. Позначимо $m = \dfrac{ab}{d}$. Ми хочемо показати, що $m = \lcm(a,b)$.
\begin{gather*}
...
\[
\dfrac{c}{m} = \dfrac{cd}{ab} = \dfrac{c}{ab}(ax+by) = \dfrac{cx}{b} + \dfrac{cy}{a} = vx+uy \implies c = m(vx+uy) \implies m \mid c.
\]
І так для кожного іншого спільного кратного. Тобто фактично ми довели, що $m = \lcm(a,b)$. Повертаючи все на місце, маємо $\lcm(a,b) = \dfrac{ab}{\gcd(a,b)}$.
\end{proof}

\begin{example}
$\lcm(392,693) = \dfrac{392 \cdot 693}{\gcd(392,693)} = \dfrac{392 \cdot 693}{7} = 38808$.\\
Значення $\gcd(392,693)$ отримали ми в \exref{gcd using Euclidean}.
\end{example}
...
\begin{align*}
\text{лише $1,\ p$ є дільниками числа $p$.}
\end{align*}
...
Число $8$ -- складене, бо окрім дільників $1,8$ ще має дільник $2,4$.
\end{example}

\begin{proposition}
Число $p$ -- просте $\iff$ якщо $p \mid ab$, то звідси або $p \mid a$, або $p \mid b$.
\end{proposition}

\begin{proof}
\rightproof Дано $p$ -- просте. Нехай $p \mid ab$. Розіб'ємо на два можливих випадки.\\
I. $p \mid a$. (насправді, ми тоді автоматично закінчимо доведення)
\bigskip \\
II. $p \nmid a$.\\
Маємо $\gcd(a,p) = 1$, але ми знаємо, що $\gcd (a,p) = ax+py = 1$ для якихось $x,y \in \mathbb{Z}$. Помножимо на $b$ -- отримаємо $abx + pby = b$. Оскільки $p \mid ab$, то звідси $ab = kp$ при $k \in \mathbb{Z}$. Звідси \[
kpx + pby = p(kx+py) = b \implies p \mid b.
\]
Отже, принаймні одне з чисел $a,b$ зобов'язано ділитись націло на $p$.
\bigskip \\
\leftproof Дано: $p \mid ab \implies p \mid a$ або $p \mid b$.\\
!Припустимо, що $p$ -- складене, тобто $p = ab$ для деяких $a > 2,\ b > 2$. Але тоді $p \mid ab$, звідси $p \mid a$ (якби був $p \mid b$, мали б аналогічну картину). Тоді $a = pq$, тому звідси $p = pqb$ -- суперечність!
\end{proof}
...
Будь-яке натуральне число $n > 1$ має єдиний розклад на добуток простих чисел з точністю до їхніх перестановок.
\end{theorem}

\begin{proof}
Доведення проведемо за МІ за числом $n \in \mathbb{N}$.\\
I. \textit{База індукції} (їх будуть аж три для розуміння теореми): \\
...
II. \textit{Припущення індукції}: для чисел $1 < k < n$ ця теорема виконується.\\
III. \textit{Крок індукції}: доведемо теорему для числа $n$. Спочатку покажемо, що взагалі-то можна розкласти, а вже згодом порушимо питання єдиності.
\bigskip \\
i. \textit{Існування}.\\
Якщо $n$ -- уже просте число, то ми закінчили доведення.\\
Тож нехай $n$ -- складене число, тоді має знайтись інший дільник $1 < a < n$, для якого $n = ab$. За припущенням МІ, оскільки $1 < a < n, 1 < b < n$, ми можемо їх розкласти на добуток простих чисел. Тобто
\begin{gather*}
a = s_1 \dots s_m,\\
b = t_1 \dots t_l
\end{gather*}
Тому отримуємо ось такий розклад: 
\[
n = s_1 \dots s_m t_1 \dots t_l,
\] всі ці числа, які в розкладі, є простими.
\bigskip \\
ii. \textit{Єдиність}. \\
...
\end{gather*}
Зауважимо, що \[
p_r \mid n \implies p_r \mid q_1 \dots q_s \implies \exists 1 \leq j \leq s: p_r \mid q_j. \]
Оскільки вони обидва прості, то звідси $p_r = q_j$.\\
Розглянемо інше число: \[
n' = p_1 \dots p_{r-1} = q_1 \dots q_{j-1} q_{j+1} \dots q_s.
\]
...
\begin{align*}
a = p_1^{r_1} \dots p_k^{r_k}; \qquad b = p_1^{s_1} \dots p_k^{s_k}.
\end{align*}
Тоді 
\begin{gather*}
\gcd (a,b) = p_1^{\min \{r_1,s_1\}} \dots p_k^{\min \{r_k,s_k\}}, \\
\lcm(a,b) = p_1^{\max \{r_1,s_1\}} \dots p_k^{\max \{r_k,s_k\}}.
\end{gather*}
\end{corollary}
...
Маємо два числа:
\[
3444 = 2^2 \cdot 3 \cdot 7 \cdot 41, \qquad 244496 = 2^4 \cdot 7 \cdot 37 \cdot 59.
\]
Тоді
\begin{gather*}
\gcd(3444, 244496) = 2^2 \cdot 7 = 28.\\
\lcm(3444, 244496) = 2^4 \cdot 3 \cdot 7 \cdot 37 \cdot 41 \cdot 59 = 30073008.
\end{gather*}
\end{example}
...
\[
d \mid n \iff d = p_1^{a_1} \dots p_r^{a_r},
\] причому $0 \leq d_i \leq k_i$.
\end{theorem}

\begin{proof}
При $d = 1$ маємо $a_i = 0, i = \overline{1,r}$, а при $d = n$ маємо $a_i = k_i, i = \overline{1,r}$.\\
Розглянемо випадок $d > 1$, тож $n = dd'$. За основною теоремою арифметики $d = q_1\dots q_s$, а також $d' = t_1 \dots t_u$. Всі ці числа прості. Отже, $p_1^{k_1} \dots p_k^{k_r} = q_1 \dots q_s t_1 \dots t_u$.\\
Оскільки розклад єдиний, то тоді кожний $q_j$ має один з $p_i$. А тому звідси ми й отримаємо, що $d = p_1^{a_1} \dots p_r^{a_r}$.\\
І навпаки, якщо $d = p_1^{a_1} \dots p_r^{a_r}$, то $n = p_1^{k_1} \dots p_k^{k_r} = (p_1^{a_1} \dots p_r^{a_r}) p_1^{k_1 - a_1} \dots p_r^{k_r - a_r}$. Отже, $d \mid n$.
\end{proof}
...
\begin{remark}
Завдяки цьому твердженню можна трохи ефективніше з'ясувати, чи буде якесь число простим.
\end{remark}
...
\begin{proof}
!Припустімо, що всього $k$ простих чисел, тобто маємо набір $p_1,p_2,\dots,p_k$. Побудуємо число \[
n = p_1 \dots p_k + 1.
\]
Розглянемо два сценарії:
\begin{enumerate}[nosep,wide=0pt,label={\arabic*)}]
\item $n$ -- просте -- автоматична суперечність нашому припущенню.
\item $n$ -- складене, тому $p_j \mid n$, бо ми можемо $n$ розкласти як добуток простих чисел. Тоді звідси $p_j \mid n - (p_1\dots p_k) = 1$. Тоді $p_j = 1$, але то вже непросте число. Суперечність! \qedhere
\end{enumerate}
\end{proof}
...
\begin{align*}
(n+1)! + 2, \quad (n+1)! + 3, \quad \dots, (n+1)! + (n+1).
\end{align*}
Їх всього $n$ штук вони послідовні та складені.
\end{proof}
...
\]
Підставляючи $x = 2^m$, одержимо
\[
(2^m)^p + 1 = (2^m+1)(2^{m(p-1)} - 2^{m(p-2)} + \dots + 1).
\]
...
\begin{remark}
Якщо $n$ -- просте, то не обов'язково $2^n-1$ -- просте число.\\ Зокрема $11$ -- просте, але $2^{11} - 1$ -- складене, оскільки \[
...
\begin{proof}
!Припустимо, що такий несталий многочлен $f(x) = \displaystyle\sum_{k=1}^N a_k x^k$ існує. Тоді $f(1) \overset{\text{позн.}}{=} p$ -- просте.\\
...
\]
Оскільки $f(1+mp)$ -- просте число, то нам треба вимагати, щоб $M=0$. В результаті \[
\forall m \in \mathbb{N}: f(1+mp) = p.
\]
А далі розглянемо многочлен $g(x) = f(x) - p$, де $g \in \mathbb{Z}[x]$. Отримаємо, що $\forall m \in \mathbb{N}: g(1+mp) = 0$. Многочлен степені $k$ може мати до $k$ коренів рівняння, а тому єдиний можливий варіант -- це $g(x) \equiv 0$, звідси $f(x) \equiv p$ -- константний многочлен. Суперечність!
\end{proof}
\newpage
...
\begin{definition}
Числа $a,b \in \mathbb{Z}$ називаються \textbf{рівними за модулем $n \in \mathbb{N}$}, якщо
\begin{align*}
...
5 = 1 \cdot 3 + 2,
\end{align*}
отже отримали однакові остачі при діленні.
\end{example}
...
\begin{proof}
\rightproof Дано: $a \equiv b \pmod n$, тоді звідси $a = nq_1 + r$ та $b = nq_2 + r$. Внаслідок чого
\[
...
\]
Із цієї рівності та умови $n \mid a-b$ випливає, що $r_1 - r_2 \mid n$, але оскільки $0 \leq r_1 < n, 0 \leq r_2 < n$, то звідси $-n < r_1 - r_2 < n$. Єдиний варіант, який нас влаштовує, -- це $r_1 - r_2 = 0 \implies r_1 = r_2$. Отримали, що $a,b$ зобов'язані мати однакову остачу при діленні на $n$, а тому $a \equiv b \pmod n$.
\end{proof}
...
\begin{example}
Розглянемо $n = 4$, як число, на яке будемо ділити. Отримаємо такі класи еквівалентностей:
\begin{align*}
...
\end{align*}
Ну а оскільки вони неперетинні, то звідси 
\[
\overline{0} \cup \overline{1} \cup \overline{2} \cup \overline{3} = \mathbb{Z}.
\]
\end{example}

\begin{definition}
Множина $\{a_1,\dots,a_n\}$ називається \textbf{повною системою лишків} за $\!\! \pmod n$, якщо
\begin{align*}
...
\begin{example}
Зокрема з попереденього прикладу $\{0,1,2,3\}$ утворюють повну систему лишків за $\!\! \pmod 4$. Однак можна взяти інші: або $\{4,6,7,9\}$ або $\{-2,-1,0,1\}$.
\end{example}

\begin{proposition}
Справедливе наступне:
\begin{enumerate}[nosep,wide=0pt,label={\arabic*)}]
\item Нехай $a \equiv b \pmod n,\ c \equiv d \pmod n$. Тоді $a + c \equiv b + d \pmod n$ (\textit{додавання});
\item Нехай $a \equiv b \pmod n,\ c \equiv d \pmod n$. Тоді $ac \equiv bc \pmod n$ (\textit{множення});
\item Нехай $a \equiv b \pmod n$, а також $d \mid n$. Тоді $a \equiv b \pmod d$ (\textit{скорочення в модулі});
\item Нехай $a \equiv b \pmod n$, а також $c \in \mathbb{N}$. Тоді $ac \equiv bc \pmod {nc}$ (\textit{множення на скаляр});
\item Нехай $a \equiv b \pmod n$. Тоді $a^m \equiv b^m \pmod n, m \in \mathbb{N}$ (\textit{зведення в степінь});
\item Нехай $ad \equiv bd \pmod n$ та $\gcd(d,n) = 1$. Тоді $a \equiv b \pmod n$ (\textit{скорочення на число в рівності}).
\item Нехай $a \equiv b \pmod c$ та $a \equiv b \pmod d$. Тоді $a \equiv b \pmod {\lcm(c,d)}$ (\textit{спільний модуль}).
\end{enumerate}
\textit{Вправа: довести (а я доведу лише передостанній пункт).}
\end{proposition}

\begin{proof}
Маємо $dx+ny = 1$, а також $n \mid d(a-b) \implies n \mid dx(a-b)$. Маємо наступне:
\begin{gather*}
n \mid (1-ny)(a-b) = (a-b) - ny(a-b) \implies n \mid a-b \implies a \equiv b \pmod n. \qedhere
\end{gather*}
\end{proof}
...
\begin{remark}
Про передостанню властивість: умова $\gcd(d,n) = 1$ є важливою для ділення. \\
Зокрема $2 \cdot 3 \equiv 2 \cdot 18 \pmod {10}$, але в жодному разі з цього НЕ випливає, що $3 \equiv 18 \pmod {10}$.
\end{remark}

\begin{corollary}
Нехай $f \in \mathbb{Z}[x]$, причому $a \equiv b \pmod n$. Тоді $f(a) \equiv f(b) \pmod n$.
\end{corollary}

Зараз наведу деякі (\textit{доволі важливі!}) приклади про квадрати та куби цілих чисел.

\begin{example}
Довести, що $x^2 \equiv 0 \pmod 4$ або $x^2 \equiv 1 \pmod 4$ для будь-якого $x \in \mathbb{Z}$.
\bigskip \\
Розглянемо $x \in \{0,1,2,3\}$. Зводячи в квадрат, одержимо $x^2 \in \{0,1,4,9\}$. Якщо від кожного взяти $\!\! \pmod 4$, отримуємо $\{0,1,1,1\} = \{0,1\}$.
\end{example}

\begin{example}
Довести, що $x^2 \equiv 0 \pmod 3$ або $x^2 \equiv 1 \pmod 3$ для будь-якого $x \in \mathbb{Z}$.\\
\textit{Приклад аналогічний, тому не розписую.}
\end{example}

\begin{example}
Довести, що $(2k+1)^2 \equiv 1 \pmod 8$ для будь-якого $k \in \mathbb{Z}$ (\textit{ліворуч непарне число}).
\bigskip \\
Розглянемо числа $\{1,3,5,7\}$, після зведення в квадрат отримуємо $\{1,9,25,49\}$, а після взяття $\!\! \pmod 8$ всюди отримуємо значення $1$.
\end{example}

\begin{example}
Довести, що $m^3 \equiv 0 \pmod 7$ або $m^3 \equiv 1 \pmod 7$ або $m^3 \equiv 6 \pmod 7$ для будь-якого $m \in \mathbb{Z}$.\\
\textit{Залишаю як вправу.}
\end{example}

\begin{example}
Довести, що $m^3 \equiv 0 \pmod 9$ або $m^3 \equiv 1 \pmod 9$ або $m^3 \equiv 8 \pmod 9$ для будь-якого $m \in \mathbb{Z}$.\\
\textit{Залишаю як вправу.}
\end{example}
...
\bigskip \\
Спочатку зауважимо, що рівняння $4k+3 = x^2$ не має цілих розв'язків. Тому що якби були розв'язки, то $x^2 \equiv 3 \pmod 4$. Приклад вище показує, що це неможливо. Тобто це означає, що жодне число виду $4k+3$ не можна представити як повний квадрат. Перефразувавши, якщо $a \equiv 3 \pmod 4$, то тоді $a$ -- неповний квадрат. Зокрема
\begin{align*}
11 \equiv 3 \pmod 4; \\
...
\subsection{Представлення числа в іншій системі числення}
\begin{theorem}
Нехай $b > 1$ -- ціле та $N \in \mathbb{N}$. Тоді число $N$ розкладається єдиним чином так:
\begin{equation}
\label{integer_representation}
...
\begin{remark}
Внаслідок чого ми можемо записувати числа в десятковій системі числення (\textit{у звичному для нас вигляді}), а також в двійковій системи числення (\textit{це більше для інформатики треба}).
\end{remark}
...
\]
У цьому випадку $0 \leq q_m < b$. Якщо покласти $a_m = q_m$, одержимо:
\[
N = a_m b^m + a_{m-1} b^{m-1} + \dots + a_2 b^2 + a_1 b + a_0.
\]
...
\end{gather*}
(\textit{насправді, не обов'язково, щоб в двох репрезентаціях були однакові індекси в кінці, але, наприклад, якщо в другій репрезентації індекс менший за $m$, то можна доповнити $c_j = 0$ до $m$-го індексу}).\\
Взявши різницю з двох репрезентацій, отримаємо
\[
0 = d_m b^m + \dots + d_1 b + d_0,\quad d_i = a_i - c_i.
\]
Оскільки репрезентації різні, то має існувати $d_k \neq 0$ (\textit{серед всіх існуючих індексів оберемо найменший можливий}). Тоді
\begin{align*}
...
Коли записуємо $N \in \mathbb{N}$ в $b$-ічну систему числення, то зазвичай пишуть ось так: 
\[
N = (a_m a_{m-1} \dots a_0)_{b},
\]
де кожний $a_i$ ще називають \textbf{цифрою}.

\begin{corollary}
Число $m$ із розкладу можна визначити із нерівності
\begin{align*}
b^m < N < b^{m+1}.
\end{align*}
Або, якщо вже дуже точно, то
\begin{align*}
m = [\log_b N].
\end{align*}
\end{corollary}
...
\begin{proof}
Перша оцінка випливає із нерівності $N > q_1$, а далі поступово розписуємо $q_1$, допоки не дістанемось до $q_m = a_m$, звідси буде $N > b^m$.\\
Друга оцінка випливає з розкладу $N$ та факту, що $0 \leq a_i \leq b-1$, тоді
\[
N \leq b^m (b-1) + b^{m-1} (b-1) + \dots + (b-1) = (b-1)(b^m + b^{m-1} + \dots + 1) = b^{m+1} - 1 < b^{m+1}.
\]
Далі, беручи $\log_b N$, одержимо нерівність
\[
m < \log_b N < m+1.
\]
Дана нерівність якраз і означає, що $m = [\log_b N]$. 
\end{proof}

\begin{theorem}
Нехай $p$ -- просте число. Тоді для кожного $a \in \mathbb{N}$ та $h \in \mathbb{N}$ справедливе
\begin{equation}
\label{eq:congruence_using_base_representation}
a^h \equiv a^{\Sigma (h)} \pmod p,
\end{equation}
де число $\Sigma (h)$ -- сума цифр числа $h$ у $p$-ічній системі числення.\\
...
Маємо 
\[
h = a_k p^k + \dots + a_1 p + a_0. 
\]
За малою теоремою Ферма відомо, що $a^p \equiv a \pmod p$, внаслідок чого $a^{d_i p^i} \equiv a^{d_i} \pmod p$. Отже,
\begin{gather*}
a^h = a^{d_k p^k} a^{d_1 p} a^{d_0} \equiv a^{d_k} \dots a^{d_1} a^{d_0} = a^{d_k + \dots + d_1 + d_0} = a^{\Sigma(h)} \pmod p. \qedhere
\end{gather*}
\end{proof}
...
5 = 0 \cdot 7 + 5.
\end{gather*}
Таким чином, $12345 = (50664)_7$. За рівнянням \eqref{eq:congruence_using_base_representation} маємо
\[
3^{12345} \equiv 3^{5 + 0 + 6 + 6 + 4} = 3^{21} \pmod 7.
\]
Можна повторити процедуру та розписати $21$ в $7$-кову систему числення. Отримаємо $21 = (30)_7$, а звідси 
\[
3^{21} \equiv 3^{3+0} = 27 \equiv 6 \pmod 7.
\]
...
%Here was a subsection{Функція $\varphi$ (функція Ойлера)}
...
Надалі ми будемо розглядати \textbf{арифметичні функції} $f \colon \mathbb{N} \to \mathbb{C}$ (хоча тут розглядатимуться частіше $\mathbb{N} \to \mathbb{N}$). Тобто це такі функції, область визначення яких -- натуральні числа, а область значень -- підмножина комплексних чисел.
...
$\displaystyle\sum_{d \mid n} f(d) = f(d_1) + \dots + f(d_r) = f\left( \dfrac{n}{d_{j_1}} \right) + \dots + f\left( \dfrac{n}{d_{j_r}} \right) \overset{\substack{\text{переставимо в} \\ \text{природному порядку}}}{=} \\ = f\left( \dfrac{n}{d_1} \right) + \dots + f\left( \dfrac{n}{d_r} \right) = \sum_{d \mid n} f\left( \dfrac{n}{d} \right)$.
\end{proof}
...
\begin{example}
Зокрема для числа $10$ ми маємо дільники $1,2,5,10$, а тому $\tau(10) = 4$, \quad $\sigma(10) = 18$.
\end{example}
...
\begin{enumerate}[nosep,wide=0pt,label={\arabic*)}]
\item $\tau(n)$ -- парне, тоді можна не перейматись.
\item $\tau(n)$ -- непарне, тоді число $n$ буде квадратом якогось числа. І дійсно, $\tau(n) = (k_1+1) \dots (k_r+1)$ -- цей вираз може бути парним тоді й тільки тоді, коли кожна дужка парна, тобто $k_i = 2s_i$. А тому $n = p_1^{k_1} \dots p_r^{r_k} = (p_1^{s_1} \dots p_1^{s_r})^2$.
\end{enumerate} 
Отже, можна взяти квадратний корінь ліворуч.
\end{proof}
...
Задані числа $m,n$ такі, що $\gcd(m,n) = 1$. Тоді множина дільників $d \mid  mn$ складається з елементів формату $d_1d_2$, де $d = d_1 \mid m,\ d_2 \mid n$, $\gcd(d_1,d_2) = 1$, а також всі $d_1d_2$ -- різні.
\end{lemma}
...
\begin{theorem}
Задано функцію $f$ -- мультиплікативна. Тоді функція $F(n) = \displaystyle\sum_{d \mid n} f(d)$ -- також мультиплікативна.
\end{theorem}
...
$F(1) = \displaystyle\sum_{d \mid 1} f(d) = f(1) = 1$.
\end{proof}
...
\begin{example}
Зокрема $\mu(45) = 0$, бо не є вільним від квадратів. Також $\mu(42) = - 1$, бо $42 = 2 \cdot 3 \cdot 7$ -- вільне від квадратів та має $3$ (непарну кількість) простих числа.
\end{example}
...
\item $m$ або $n$ -- не вільне від квадратів. Тобто існує просте число $p$, для якого $p^2 \mid m$ або $p^2 \mid n$. В обох випадках отримаємо $p^2 \mid mn$, а тому також $mn$ не буде вільним від квадратів. Отже, $\mu(mn) = 0 = \mu(m)\mu(n)$.
\item $m$ та $n$ -- вільні від квадратів, нехай $m = p_1 \dots p_r$ та $n = q_1 \dots q_s$, причому $p_i,q_j$ всі різні в силу $\gcd(m,n) = 1$. Отже, $mn = p_1 \dots p_r q_1 \dots q_s$, а тому $\mu(mn) = (-1)^{r+s} = (-1)^r (-1)^s = \mu(m)\mu(n)$.
\end{enumerate}
$\mu(1) = 1$ із означення. Довели мультиплікативніть.
\end{proof}
...
$f(1) = \displaystyle\sum_{d \mid 1} \mu(d) F\left(\dfrac{1}{d}\right) = \mu(1) F(1) = 1$.
\end{proof}
...
\begin{proof}
Із того, що $\gcd(a,n) = 1,ґ \gcd(b_i,n) = 1$ випливає $\gcd(ab_i,n) = 1$.\\
...
!Припустимо, що існує просте $p$, для якого $p \mid cy, p \mid m$. Тоді $p \mid c,\\  p \mid m \implies p \mid c,\ p \mid mn$ (суперечність), або $p \mid y,\ p \mid m$ (суперечність)!\\
...
\begin{proof}
Маємо $n$. Серед перших $n$ чисел, що діляться на $p$, є $p,2p,\dots,tp$. Тут $t$ -- найбільше ціле число, для якого $tp \leq n$. Тобто $t \leq \dfrac{n}{p}$ та $t$ -- таке найбільше. Отже, $t = \left[ \dfrac{n}{p} \right]$. Тобто всього рівно $\left[ \dfrac{n}{p} \right]$ чисел, що кратні $p$, що з'являються в $n!$, а саме: $p,2p,\dots, \left[ \dfrac{n}{p} \right]p$ $(1)$.\\
...
Ми отримали ще додатково $\left[ \dfrac{n}{p^2} \right]$ простих чисел $p$, тобто загалом вже їхня кількість дорівнює $\left[ \dfrac{n}{p} \right] + \left[ \dfrac{n}{p^2} \right]$.\\
...
Тобто всього $\left[ \dfrac{N}{k} \right]$ дільників $k$, це теж саме, що $f(k)$ знаходиться в $\left[ \dfrac{N}{k} \right]$ різних сумах формата $\displaystyle\sum_{d \mid n} f(d)$. Отже, можемо переписати вираз:\\
...
\subsection{Порядок}
\begin{definition}
Задані $a,n \in \mathbb{N}$ -- взаємно прості.\\
...
\begin{proposition}
\label{order_and_numbers_bigger_than_order}
...
\begin{remark}
Якщо $n,a$ -- взаємно прості, то тоді звідси $n,a^k$ -- також взаємно прості для всіх $k \in \mathbb{N}$, тож $\ord_n(a^k)$ можна визначити.
\end{remark}
...
Виникає важливе питання, коли взагалі первісні корені існують. Відповідь дає ось така теорема.
...
Нехай $p$ -- просте число. Тоді існує первісний корінь $\pmod p$.
\end{theorem}

Але, насправді, ми доведемо більш загальну теорему:
\begin{theorem}
Якщо $d \mid p-1$, то тоді всього $\varphi(d)$ неконгруентних чисел порядка $d$ за $\!\! \pmod p$.
\end{theorem}
...
\begin{proof}
Оскільки $r$ -- первісний корінь за $\!\!\pmod p$, то $r^{p-1} \equiv 1 \pmod p$. Тобто $r^{p-1} = 1 + px, x \in \mathbb{Z}$.\\
...
Коротше, ми маємо $a^{2^{k-2}} \equiv 1 \pmod {2^k}$, а тому  $a^{2^{k-2}} = 1 + x \cdot 2^k$ для деякого $x \in \mathbb{Z}$.\\
Взведемо обидві частини в квадрат:\\
$a^{2^{k-1}} = 1 + x \cdot 2^{k+1} + x^2 \cdot 2^{2k} \equiv 1 \pmod {2^{k+1}}$.\\
У нас виник степінь менший за $\varphi(2^{k+1})$, для якого виконується конгруенція $1$ для будь-якого непарного числа. А тому первісних коренів $\pmod {2^{k+1}}$ не має.\\
МІ доведено.
\end{proof}
...
Тоді оскільки $\gcd(m_1,m_2) = 1$, маємо $a^{\frac{\varphi(n)}{2}} \equiv 1 \pmod {n}$. І знову отримали степінь менший за $\varphi(n)$, де конгруенція один.
\end{proof}
...
Нехай $p \mid F_n$, тобто $F_n \equiv 0 \pmod p$, або маємо $2^{2^n} \equiv -1 \pmod p$. Звідси $(2^{2^n})^2 = 2^{2^{n+1}} \equiv 1 \pmod p$. Отже, за \prpref{order_and_numbers_bigger_than_order} $\ord_p(2) \mid 2^{n+1}$. Водночас за $\!\! \pmod p$ первісний корінь завжди існує, тож $\ord_p(2) = \varphi(p) = p-1$. Внаслідок чого $p -1 \mid 2^{n+1}$, а це й означає $p = 1 + x 2^{n+1}$ для деякого $x \in \mathbb{N}$.
\end{example}
...
\begin{definition}
Задані $a,n$ -- взаємно прості, $r$ -- первісний $\!\! \pmod n$.\\
...
\section{Квадратичні лишки}
\subsection{Вступ до квадратичних лишків}
\begin{definition}
...
\end{align*}
Власне, символ Лежандра просто дає відповідь, чи буде число $a$ квадратичним лишком за $\!\! \pmod p$.
\end{definition}
...
\end{enumerate}
\textit{Усі випливають з критерія Ойлера. У принципі, довести неважко.}
\end{corollary}
...
\begin{example}
Нехай $p$ -- непарне просте число та $a$, де $p \nmid a$. Розглянемо діофантове рівняння нижче.\\
$x^2 + py + a = 0$ має цілий розв'язок $(x_0,y_0) \iff \left( \dfrac{-a}{p} \right) = 1$.
\bigskip \\
\rightproof Дано: $x_0^2 + py_0^2 + a = 0$, тобто є розв'язок. Звідси $0 = x_0^2 + py_0 + a \equiv x_0^2 + a \pmod p$. Звідси випливає, що $x_0^2 \equiv -a \pmod p$, тобто $(-a)$ -- квадратичний лишок $\!\! \pmod p$. Отже, $\left( \dfrac{-a}{p} \right) = 1$.
\bigskip \\
\leftproof Дано: $\left( \dfrac{-a}{p} \right) = 1$. Значить, існує розв'язок $x^2 \equiv -a \pmod p$. Позначимо його за $x_0$. Звідси $x_0^2 + a = pk$ для деякого $k \in \mathbb{Z}$. Звідси $x_0^2 - pk + a = 0$. Якщо позначити $y_0 = -k$, то отримуємо бажане.  
\end{example}
...
\begin{theorem}
Кількість простих чисел формату $4k+1$ -- нескінченна.
\end{theorem}

\begin{proof}
!Припустимо, що їх скінченна кількість. Маємо лише $p_1,p_2,\dots,p_k$ -- всі прості числа формату $4k+1$. Іншими словами, всі вони $\equiv 1 \pmod 4$.\\
Розглянемо число $N = (2p_1\dots p_k)^2 + 1$. Воно за припущенням буде складеним. Також $N$ -- непарне, тож існує непарне просте число $p \mid N$. Звідси $N \equiv 0 \pmod p$, тобто $(2p_1 \dots p_k)^2 \equiv -1 \pmod p$. Отже, $-1$ -- квадратичний лишок $\!\! \pmod p$, тобто $\left( \dfrac{-1}{p} \right) = 1$. Але даний символ Лежандра одиниця, якщо $p \equiv 1 \pmod 4$. Висновок: просте число $p = p_i$ -- один з простих чисел в списку вище.\\
$p_i \mid N$ та $p_i \mid ( 2 p_1 \dots p_k)^2$, значить $p_i \mid 1$ -- суперечність!
\end{proof}

\subsection{Закон квадратичної взаємності}
...
\begin{remark}
Практичніше буде використовувати такі зауваження:\\
якщо $p \equiv 1 \pmod 4$ або $q \equiv 1 \pmod 4$, то за наслідком $\left( \dfrac{p}{q} \right) = \left( \dfrac{q}{p} \right)$;\\
якщо $p \equiv 3 \pmod 4$ та $q \equiv 3 \pmod 4$, то за наслідком $\left( \dfrac{p}{q} \right) = -\left( \dfrac{q}{p} \right)$.
\end{remark}
...
$\boxed{=} - \left( \dfrac{11}{101} \right) \overset{101 \equiv 1 \pmod 4}{=} -\left( \dfrac{101}{11} \right) = -\left( \dfrac{2}{11} \right) \overset{11 \equiv 3 \pmod 8}{=} -(-1) = 1$.
\end{example}
...
\subsection{Всі можливі прості числа, при яких деяке число -- квадратичний лишок}
Нехай $a \in \mathbb{N}$. Поставимо таке питання: при яких простих $p$ число $a$ буде квадратичним лишком за $\!\! \pmod {p}$?\\
Почнемо з випадку, коли $a = q$ -- просте число.

\begin{theorem}
Нехай $q \equiv 1 \pmod 4$. Тоді\\
$q$ -- квадратичний лишок $\!\! \pmod p \iff p \equiv r \pmod {q}$, де $r$ -- квадратичний лишок $\pmod q$. 
\end{theorem}

\begin{proof}
$q$ -- квадратичний лишок $\!\! \pmod p \iff  \left( \dfrac{q}{p} \right) = 1 \overset{\substack{\text{з-н квадратичної} \\ \text{взаємності}}}{\iff} \\ \iff \left( \dfrac{p}{q} \right) = 1 \iff p$ -- квадратичний лишок $\!\! \pmod q \iff p \equiv r \pmod q$, де $r$ -- квадратичний лишок $\pmod q$.
\end{proof}

\begin{example}
Розглянемо $q = 5$, яке потрапляє в умову $q \equiv 1 \pmod 4$. Ми вже знаємо, при яких $p$ це число $q$ буде квадратичним лишком $\!\! \pmod p$. Нам треба взяти всі квадратичні лишки за $\!\! \pmod q$, це будуть наші $p$.\\
За $\!\! \pmod q$, тобто $\!\! \pmod 5$ у нас числа $1,4$ -- квадратичні лишки.
\bigskip \\
Отже, $\left( \dfrac{5}{p} \right) = \begin{cases} 1, & p \equiv 1 \pmod 5 \text{ або } p \equiv 4 \pmod 5 \\ -1, & p \equiv 2 \pmod 5 \text{ або } p \equiv 3 \pmod 5  \end{cases}$. 
\end{example}

\begin{theorem}
Нехай $q \equiv 3 \pmod 4$. Тоді\\
$q$ -- квадратичний лишок $\!\! \pmod p \iff p \equiv \pm b^2 \pmod {4q}$, де $b$ -- непарне число, причому $q \nmid b$. 
\end{theorem}

\begin{remark}
Окремо зроблю вставку. Нехай $q \equiv 3 \pmod 4$. Тоді \\ $s$ -- квадратичний нелишок $\!\!\pmod q \iff (-s)$ -- квадратичний лишок $\!\!\pmod q$.
\end{remark}

\begin{proof}
\rightproof Дано: $q$ -- квадратичний лишок $\!\! \pmod p$, тобто $\left( \dfrac{q}{p} \right) = 1$. Закон про квадратичну взаємність каже, що $\left( \dfrac{q}{p} \right) = 1 = (-1)^{\frac{p-1}{2}} \left( \dfrac{p}{q} \right)$. Існують два випадки, які треба розглянути ретельно.
\begin{enumerate}[nosep,wide=0pt,label={\arabic*)}]
\item Нехай $(-1)^{\frac{p-1}{2}} = -1$, а також $\left( \dfrac{p}{q} \right) = -1$. Із цих умов випливають, що $p \equiv 3 \pmod 4$, а також $p$ -- нелишок $\!\! \pmod q$. Але за зауваженням $(-p)$ буде лишком $\!\! \pmod q$, тож існує число $b$, для якого $b^2 \equiv -p \pmod q$. Можемо припускати, що $b$ -- непарне. Адже якщо $b$ -- парне, то замінимо на $b' = b+q$. Дане припущення дає нам $b^2 \equiv 1 \pmod 4$, тобто $p \equiv 3 \equiv -b^2 \pmod 4$. Оскільки ще $p \equiv -b^2 \pmod q$, то звідси $p \equiv -b^2 \pmod {4q}$.

\item Нехай $(-1)^{\frac{p-1}{2}} = 1$, а також $\left( \dfrac{p}{q} \right) = 1$. Із двох умов випливають, що $p \equiv 1 \pmod 4$, а також $p$ -- квадратичний лишок $\!\! \pmod q$, тобто існує $b$ (яке аналогічно припускається як непарне), для якого $p \equiv b^2 \pmod q$. Тоді миттєво $p \equiv b^2 \pmod {4q}$. 
\end{enumerate}

\leftproof Дано: $p \equiv \pm b^2 \pmod {4q}$, де $b$ -- непарне.\\
Якщо $p \equiv b^2 \pmod {4q}$, то звідси $\begin{cases} p \equiv b^2 \overset{b \text{ -- непарне}}{\equiv} 1 \pmod 4 \\ p \equiv b^2 \pmod q \end{cases}$. Перше рівняння каже, що $(-1)^{\frac{p-1}{2}} = 1$, а друге -- що $\left( \dfrac{p}{q} \right) = 1$. Тоді за квадратичною взаємністю $\left( \dfrac{q}{p} \right) = 1$.\\
Якщо $p \equiv -b^2 \pmod {4q}$, то десь аналогічно $\left( \dfrac{q}{p} \right) = 1$.
\end{proof}

\begin{example}
Розглянемо число $q = 3$, яке потрапляє в умову $q \equiv 3 \pmod 4$. Ми вже знаємо, при яких $p$ це число $q$ буде квадратичним лишком $\!\! \pmod p$. Нам треба взяти всі можливі $b^2$ за $\!\! \pmod {4q}$, тобто $\!\! \pmod {12}$, де $b$ -- непарне число, яке не ділиться на $3$. Числа $1^2,5^2,7^2,11^2$ -- всі вони $\equiv 1 \pmod {12}$.
\bigskip \\
Отже, $\left( \dfrac{3}{p} \right) = \begin{cases} 1, & p \equiv \pm 1 \pmod {12} \\ -1, & p \equiv \pm 5 \pmod {12} \end{cases}$.
\end{example}

\subsection{Символ Якобі}
\begin{definition}
Нехай $a$ -- будь-яке число та $b$ -- непарне додатне число, нехай його розклад $b = p_1 p_2 \dots p_m$ (тут не обов'язково різні).\\
\textbf{Символ Якобі} визначається таким чином:
\begin{align*}
\left( \dfrac{a}{b} \right) = \left( \dfrac{a}{p_1} \right) \left( \dfrac{a}{p_2} \right) \dots \left( \dfrac{a}{p_m} \right)
\end{align*}
У правій частині стоять символи Лежандра за кожним простим.
\end{definition}

\begin{proposition}
Нехай символ Якобі $\left( \dfrac{a}{b} \right) = -1$. Тоді $a$ -- квадратичний нелишок $\!\! \pmod b$.
\textit{Вправа: довести.}
\end{proposition}

\begin{remark}
Якщо символ Якобі $\left( \dfrac{a}{b} \right) = 1$, то \textcolor{red}{не обов'язково} $a$ -- квадратичний лишок $\!\! \pmod b$.
\end{remark}

\begin{example}
Зокрема $\left( \dfrac{2}{15} \right) = \left( \dfrac{2}{3} \right) \left( \dfrac{2}{5} \right) = (-1)(-1) = 1$. Проте можна перевірити, що $2$ -- квадратичний нелишок $\!\! \pmod {15}$.
\end{example}

\begin{proposition}[Властивості]
Справедливе наступне:
\begin{enumerate}[nosep,wide=0pt,label={\arabic*)}]
\item $a_1 \equiv a_2 \pmod b \implies \left( \dfrac{a_1}{b} \right) = \left( \dfrac{a_2}{b} \right)$;
\item $\left( \dfrac{a_1 a_2}{b} \right) = \left( \dfrac{a_1}{b} \right) \left( \dfrac{a_2}{b} \right)$;
\item $\left( \dfrac{a}{b_1 b_2} \right) = \left( \dfrac{a}{b_1}\right) \left( \dfrac{a}{b_1}\right)$.
\end{enumerate}
\textit{Перші два випливають з властивостей символа Лежандра, третя легко доводиться.}
\end{proposition}

\begin{proposition}
$\left( \dfrac{-1}{b} \right) = (-1)^{\frac{b-1}{2}}$.
\end{proposition}

\begin{proof}
Нехай $b = p_1 \dots p_m$ -- розклад на прості множники (не обов'язково різні). Тоді\\
$\left( \dfrac{-1}{b} \right) = \left( \dfrac{-1}{p_1} \right) \dots \left( \dfrac{-1}{p_m}\right) = (-1)^{\frac{p_1-1}{2}} \dots (-1)^{\frac{p_m-1}{2}} = (-1)^{\frac{p_1 -1}{2} + \dots + \frac{p_m -1}{2}} \textcolor{red}{\overset{?}{=}} \\ = (-1)^{\frac{p_1\dots p_m-1}{2}} = (-1)^{\frac{b-1}{2}}$.
\bigskip \\
Червона рівність буде правдивою, тільки треба довести наступне: якщо $r,s$ -- непарні числа, то $\dfrac{rs-1}{2} \equiv \dfrac{r-1}{2} + \dfrac{s-1}{2} \pmod 2$ (це можна узагальнити для кількох непарних простих за МІ).\\
Оскільки $r,s$ -- прості, то $(r-1)(s-1) \equiv 0 \pmod 4$, а тому $rs - 1 \equiv (r-1) + (s-1) \pmod {4}$, а тому звідси $\dfrac{r-1}{2} + \dfrac{s-1}{2} \equiv \dfrac{rs-1}{2} \pmod {4}$.
\end{proof}

\begin{proposition}
$\left( \dfrac{2}{b} \right) = (-1)^{\frac{b^2-1}{2}}$.
\end{proposition}

\begin{proof}
Нехай $b = p_1 \dots p_m$ -- розклад на прості множники (не обов'язково різні). Тоді\\
$\left( \dfrac{-2}{b} \right) = \left( \dfrac{-2}{p_1} \right) \dots \left( \dfrac{-2}{p_m}\right) = (-1)^{\frac{p^2_1-1}{8}} \dots (-1)^{\frac{p^2_m-1}{8}} = (-1)^{\frac{p^2_1 -1}{8} + \dots + \frac{p^2_m -1}{2}} \textcolor{red}{\overset{?}{=}} \\ = (-1)^{\frac{p^2_1\dots p^2_m-1}{8}} = (-1)^{\frac{b^2-1}{8}}$.
\bigskip \\
Червона рівність буде правдивою, тільки треба довести наступне: якщо $r,s$ -- непарні числа, то $\dfrac{r^2 s^2-1}{8} \equiv \dfrac{r^2-1}{8} + \dfrac{s^2-1}{8} \pmod 2$ (це можна узагальнити для кількох непарних простих за МІ).\\
Оскільки $r,s$ -- непарні, то $4 \mid r^2-1$ та $4 \mid s^2-1$, тож $(r^2-1)(s^2-1) \equiv 0 \pmod {16}$, а тому $r^2s^2 - 1 \equiv (r^2-1) + (s^2-1) \pmod {16}$, а тому звідси $\dfrac{r^2-1}{8} + \dfrac{s^2-1}{8} \equiv \dfrac{rs-1}{8} \pmod {16}$.
\end{proof}

\begin{theorem}[Узагальнений квадратичний закон взаємності]
Нехай $a,b$ -- обидва додатні непарні числа. Тоді $\left( \dfrac{a}{b} \right) \left( \dfrac{b}{a} \right) = (-1)^{\frac{a-1}{2} \frac{b-1}{2}}$.
\end{theorem}

\begin{proof}
Нехай в нас такі розклади на прості (не обов'язково різні):\\
$a = q_1 \dots q_l$;\\
$b = p_1 \dots p_m$.\\
Звідси $\left( \dfrac{a}{b} \right) \left( \dfrac{b}{a} \right) = \displaystyle\prod_i \prod_j \left( \dfrac{q_i}{p_j} \right) \left( \dfrac{p_j}{q_i} \right) = (-1)^{\sum_i \sum_j \frac{q_1-1}{2} \frac{p_j-1}{2}}$.\\
Утім в позапопередньому твердженні був факт на останньому абзаці, який можна тут використати. Зокрема\\
$\displaystyle\sum_i \sum_j \dfrac{q_i-1}{2} \dfrac{p_j-1}{2} \equiv \dfrac{a-1}{2} \sum_j \dfrac{p_j - 1}{2} \equiv \dfrac{a-1}{2} \dfrac{b-1}{2} \pmod 2$.\\
Отже, $\left( \dfrac{a}{b} \right) \left( \dfrac{b}{a} \right) = (-1)^{\frac{a-1}{2} \frac{b-1}{2}}$.
\end{proof}

\begin{theorem}
Нехай $a$ -- число, що не є квадратом. Тоді існує нескінченне число простих чисел $p$, для яких $a$ -- квадратичний нелишок $\!\!\pmod p$. 
\end{theorem}

\begin{remark}
Досить цю теорему довести для випадку, коли $a$ -- вільне від квадратів.\\
Якщо $\tilde{a}$ -- інше число, що не квадрат, то можна записати як $\tilde{a} = c^2 a$, де $a$ -- вільне від квадратів. Далі просто треба розписати через символ Якобі.
\end{remark}

\begin{proof}
Отже, маємо $a$ -- вільне від квадратів, тобто $a = 2^e q_1 \dots q_n$, де всі $q_i$ -- різні непарні прості та $e \in \{0,1\}$.\\
I. \textit{Випадок, коли $a > 2$.}\\
Тобто тоді існує непарний простий множник в розкладі.\\
!Припустимо, що в нас скінченна кількість простих чисел, для яких $a$ -- квадратичний нелишок $\pmod p$. Серед них оберемо ті прості числа, які не потрапили в розклад $a$. Це буде множина $\{l_1,\dots,l_k\}$.\\
Оскільки $q_n$ -- непарне просте, то існує квадратичний нелишок $s$. Далі розглянемо систему:
$\begin{cases}
x \equiv 1 \pmod {l_j},\ j = 1,\dots,k \\
x \equiv 1 \pmod {8} \\
x \equiv 1 \pmod {q_i},\ i = 1,\dots,n-1 \\
x \equiv s \pmod {q_n}
\end{cases}$.\\
За китайською теоремою про остачі маємо розв'язок системи $b$. Число $b$ непарне в силу другого рівняння (тому можна символ Якобі використати). Нехай $b = p_1 \dots p_m$ має такий розклад.\\
Оскільки $b \equiv 1 \pmod {8}$, то $\left( \dfrac{2}{b} \right) = 1$ та $\left( \dfrac{q_i}{b} \right) = \left( \dfrac{b}{q_i} \right)$. Отже,\\
$\left( \dfrac{a}{b}\right) = \left( \dfrac{2}{b} \right)^e \left( \dfrac{q_1}{b}\right) \dots \left( \dfrac{q_{n-1}}{b} \right) \left( \dfrac{q_n}{b} \right) = \left( \dfrac{b}{q_1} \right) \dots \left( \dfrac{b}{q_{n-1}} \right) \left( \dfrac{b}{q_n} \right) =
\\ = \left( \dfrac{1}{q_1} \right) \dots \left( \dfrac{1}{q_{n-1}} \right) \left( \dfrac{s}{q_n} \right) = -1$.\\
Із іншого боку, маємо\\
$\left( \dfrac{a}{b} \right) = \left( \dfrac{a}{p_1} \right) \dots \left( \dfrac{a}{p_m} \right)$.\\
Внаслідок чого $\left( \dfrac{a}{p_i} \right) = -1$ для деякого $i$. Із першого рівняння системи випливає, що жодний $l_j \nmid b$. Значить, жодний $l_j \neq p_i$, тоді звідси $p_i \notin \{l_1,\dots,l_k\}$.\\
Отже, для вільного від квадрату числа $a > 2$ існує просте число $p \notin \{2,l_1,\dots,l_k\}$, для якого $\left( \dfrac{a}{p} \right) = -1$ -- суперечність!
\bigskip \\
II. \textit{Випадок, коли $a = 2$.}\\
Робимо те аме припущення. Беремо схожу множину $\{l_1,\dots,l_k\}$, але викинемо $3$. Розглянемо число $b = 8 l_1 \dots l_k + 3$, яке не ділиться на $3$ та на жодний $l_j$. Оскільки $b \equiv 3 \pmod 8$, то $\left( \dfrac{2}{b}\right) = (-1)^{\frac{b^2-1}{8}} = -1$. Далі розкладемо $b = p_1 \dots p_m$. Тоді аналогічними міркуваннями отримуємо $\left( \dfrac{2}{p_i} \right) = -1$ для деякого $i$, тому так само $p_i \notin \{3,l_1,\dots,l_k\}$ -- така сама суперечність. 
\end{proof}
...
\begin{lemma}
Задані $n,m$ -- числа, які записуються як сума двох квадратів. Тоді $mn$ теж записується як сума двох квадратів.
\end{lemma}

\begin{proof}
Маємо ось такі репрезентації: $n = a^2+b^2$, \quad $m = c^2+d^2$.\\
...
!Припустимо, що умова $p \mid x$ та $p \mid y$ не виконуються. Є три випадки:
\begin{enumerate}[nosep,wide=0pt,label={\Roman*.}]
...
\begin{corollary}
Кожне просте число $p \equiv 1 \pmod 4$ розписується як сума двох квадратів єдиним чином з точністю до перестановки доданків. Ще припускається, що $a^2 = (-a)^2$ -- це один випадок.
\end{corollary}
...
\begin{lemma}[Лема Ойлера]
Задано $n,m$ -- числа, які записуються як сума чотирьох квадратів. Тоді $mn$ теж записується як сума чотирьох квадратів.\\
\textit{Доведення не дуже чесне. Для демонстації з голови треба знати кватерніони, а це треба підключати апарат абстрактної алгебри.}
\end{lemma}
...
\begin{enumerate}[nosep,wide=0pt,label={\Roman*.}]
\item $u = x^2$ та $v = y^2$. У нас вже $x^2 \equiv y^2 \pmod p$, але звідси або $x \equiv y \pmod p$, або $x \equiv -y \pmod p$ -- жодний з двох варіантів неможливий, бо $0 \leq x \leq \dfrac{p-1}{2}$, $0 \leq y \leq \dfrac{p-1}{2}$.
\item $u = -1-x^2$ та $v = -1-y^2$ -- аналогічно неможливо за 1).
\item $u = x^2$ та $v = -1-y^2$ (або навпаки). Тоді звідси $x^2+y^2+1 \equiv 0$ при обмеженнях $x,y$.
\end{enumerate}
У трьох випадках довели бажане.
\end{proof}
...
Із всіх варіантів залишається лише $k = 1$.
\end{proof}
...
Тут активно застосовується лема Ойлера.\\
...
\begin{definition}
Нехай задано $[a_0;a_1,\dots,a_n]$. Зробимо позначення:
\begin{align*}
C_k = [a_0;a_1,\dots,a_k],\ 0 \leq k \leq n,
\end{align*}
тобто $C_k$ -- ланцюговий дріб, що на початку, але без частини після $a_k$.\\
...
Мені подобається $C_k$ називати \textbf{$k$-тим збіжником}.
\end{definition}
...
Крок індукції: нехай для будь-якого ланцюгового дробу та для $2 \leq m < n$ виконується теорема, зокрема й рівність \\ $C_m = [a_0;a_1,\dots,a_{m-1},a_m] =\dfrac{p_m}{q_m} = \dfrac{a_m p_{m-1} + p_{m-2}}{a_m q_{m-1} + q_{m-2}}$. \\
Доведемо це твердження для $C_{m+1}$.\\
...
Коротше, збіжник $C_k = \dfrac{p_k}{q_k}$ уже нескоротимий дріб.
\end{corollary}
...
\begin{proof}
Із попереднього підрозділу, маємо такий ланцюг нерівностей:\\
$C_0 < C_2 < C_4 < \dots < C_{2n} < C_{2n+1} < \dots < C_5 < C_3 < C_1$.\\
Таким чином, послідовність $\{C_{2n}, n \geq 1\}$ не лише монотонно зростає, а ще й обмежена сверху числом $C_1$; послідовність $\{C_{2n+1}, n \geq 1\}$ не лише монотонно спадає, а ще й обмежена знизу числом $C_0$. Отже, обидва мають границю за Ваєрштрасом: $\alpha = \displaystyle\lim_{n \to \infty} C_{2n}$ та $\alpha' = \displaystyle\lim_{n \to \infty} C_{2n+1}$. \\
Зауважимо, що $\alpha' \leq C_{2n+1}$ та $\alpha \geq C_{2n}$. Отже, маємо\\
...
Зрозуміло, що $q_k$ строго зростає, а тому й $q_{2n}$. Таким чином, $\forall \varepsilon > 0: \exists N: 0 \leq |\alpha'-\alpha| < \dfrac{1}{q_{2N}^2} < \varepsilon$. Звідси маємо $\alpha = \alpha'$. Звідси абсолютно всі часткові границі послідовності $\{C_n, n \geq 0\}$ дорівнюють $\alpha$, а тому звідси $\displaystyle\lim_{n \to \infty} C_n = \alpha$.
\end{proof}
...
$0 < \left| \dfrac{a}{b} - \dfrac{p_n}{q_n} \right| < \dfrac{1}{q_n q_{n+1}} \implies 0 < |aq_n - bp_n| < \dfrac{b}{q_{n+1}}$.\\
...
\begin{corollary}
Два різних ланцюгових дроби представляють два різних ірраціональних числа.
\end{corollary}
...
\begin{example}
За даним алгоритмом можна показати наступне:\\
...
\subsection{Періодичний ланцюговий дріб}
\begin{definition}
...
\begin{theorem}
$x \not\in \mathbb{Q}$ має періодичний ланцюговий дріб $\iff$ $x$ має формат числа $r + s\sqrt{d}$, де $r,s \in \mathbb{Q}$ та $s \neq 0$, а також $d$ -- вільне від квадратів.
\end{theorem}

\begin{definition}
Число формата $r + s \sqrt{d}$ при $r,s \in \mathbb{Q},\ s \neq 0$ та $d$ -- вільне від квадрата, називають \textbf{квадратичне ірраціональним числом}.
\end{definition}

\subsection{Оцінки для ланцюгових дробів}
\begin{lemma}
...
\section{Постулат Бертрана}
\begin{theorem}
Для кожного цілого числа $n \geq 2$ існує щонайменше одне просте число $p$, для якого $n < p < 2n$.
\end{theorem}

Даний постулат вже можна вважати за теорему, бо була доведена Чебишовим.

\begin{lemma}
Для кожного $n \in \mathbb{N}$ виконується $\dfrac{4^n}{2n} \leq C_{2n}^n$.
\end{lemma}

\begin{proof}
$4^n = (1+1)^{2n} = \displaystyle\sum_{k=0}^{2n} C_{2k}^k = 2 + \sum_{k=1}^{2n-1} C_{2k}^k \leq 2n C_{2n}^n$.
\end{proof}

\begin{lemma}
Зафіксуємо просте число $p$. Визначимо $R = R(n,p)$ -- найбільше натуральне число $r$ таке, що $p^r \mid C_{2n}^n$. Тоді для кожного простого $p$ виконується $p^R \leq 2n$. 
\end{lemma}

\begin{proof}
Для числа $n!$ степінь простого числа $p$ визначається за формулою Лежандра $\displaystyle\sum_{j=1}^\infty \left[ \dfrac{n}{p^j} \right]$. Тоді для числа $C_{2n}^n = \dfrac{(2n)!}{n!n!}$ степінь простого числа $p$ дорівнює $\displaystyle\sum_{j=1}^\infty \left[ \dfrac{2n}{p^j} \right] - 2 \sum_{j=1}^\infty \left[ \dfrac{n}{p^j} \right]$. Такий степінь є найбільшим можливим, тому\\
$\displaystyle\sum_{j=1}^\infty \left( \left[ \dfrac{2n}{p^j} \right] - 2 \left[ \dfrac{n}{p^j} \right] \right) = R$.\\
У цій сумі при $j > \log_p(2n)$ всі доданки нулеві. Інакше зауважимо, що $\left[ \dfrac{2n}{p^j} \right] - 2 \left[ \dfrac{n}{p^j} \right]$ приймає значення $0$ або $1$. Таким чином, $R \leq \log_p (2n)$. Власне, звідси $p^R \leq 2n$.
\end{proof}

\begin{lemma}
Нехай $p$ -- таке непарне просте число, що $\dfrac{2n}{3} < p \leq n$. Тоді $R(n,p) = 0$.
\end{lemma}

\begin{proof}
Ми вже знаємо, що $\displaystyle\sum_{j=1}^\infty \left( \left[ \dfrac{2n}{p^j} \right] - 2 \left[ \dfrac{n}{p^j} \right] \right) = R$ та що $\left[ \dfrac{2n}{p^j} \right] - 2 \left[ \dfrac{n}{p^j} \right]$ приймає значення $0$ або $1$ -- це було в лемі 2. \\
Припустимо, що $\dfrac{2n}{p^j} \geq 1$, тобто $p^j \leq 2n \implies j \leq \log_p 2n < \log_p (3p) = 1 + \log_p 3 \overset{\text{$p$ -- непарне}}{<} 2$. Єдиний варіант тут -- це $j = 1$. У силу нашої оцінки на просте число $p$ матимемо $\left[ \dfrac{2n}{p^j} \right] - 2 \left[ \dfrac{n}{p^j} \right] = 2 - 2 \cdot 1 = 0$.\\
Якщо $\dfrac{2n}{p^j} < 1$, то тим паче $\dfrac{n}{p^j} < \dfrac{1}{2}$, а тому $\left[ \dfrac{2n}{p^j} \right] - 2 \left[ \dfrac{n}{p^j} \right] = 0$.\\
Внаслідок чого отримали $R = 0$. 
\end{proof}

\begin{lemma}
Визначимо $n \# = \displaystyle\prod_{p \leq n} p$ -- це називають англійською primorial. Для кожного $n \geq 1$ виконується $n \# < 4^n$.
\end{lemma}

\begin{proof}
I. \textit{База індукції}. При $n = 1$ маємо $1 \# = 1 < 4$.\\
II. \textit{Припущення індукції (перша частина)}. При $1 \leq n \leq 2k-1$ нерівність виконується.\\
III. \textit{Крок індукції (перша частина)}. Число $n = 2k > 2$ та є складеними, а тому звідси $(2k)\# = (2k-1)\# \overset{\text{припущення МІ}}{<} 4^{2k-1} < 4^{2k}$.
\bigskip \\
II. \textit{Припущення індукції (друга частина)}. При $1 \leq n \leq 2k$ нерівність виконується.\\
III. \textit{Крок індукції (друга частина)}. Зауважимо, що $C_{2k+1}^k$ -- ціле число, при цьому прості числа $k+2 \leq p \leq 2k+1$ з'являються лише в $(2k+1)!$ (тобто в чисельнику). Звідси\\
$\dfrac{(2k+1)\#}{(k+1)\#} \leq C_{2k+1}^k = \dfrac{1}{2} (C_{2k+1}^k + C_{2k+1}^{k+1}) \leq \dfrac{1}{2} (1+1)^{2k+1} = 4^k$.\\
Отже, $(2k+1)\# = (k+1)\# \cdot \dfrac{(2k+1)\#}{(k+1)\#} \overset{\text{припущення МІ}}{<} 4^k \cdot 4^{k+1} = 4^{2k+1}$.\\
МІ доведено.
\end{proof}

Тепер ми готові довести постулат Бертрана.

\begin{proof}
!Припустимо, що існує натуральне число $n \geq 2$, таке, щоб не існувало простого числа $p$, де $n < p < 2n$. Тоді обов'язково $n \geq 427$.\\
Якби $n < 427$, то ми можемо обрати просте число $p$ серед цього списку: $\{3,5,7,13,23,43,83,163,317,631\}$. Тому обов'язково $n \geq 427$.\\
Розглянемо число $C_{2n}^n$. Тут в нас така ситуація: не існує простого множника так, що:
\begin{itemize}[nosep,wide=0pt]
\item $2n < p$ (кожний множник має ділити $(2n)!$);
\item $p = 2n$ ($2n$ взагалі не є простим);
\item $n < p < 2n$ (припущення);
\item $\dfrac{2n}{3} < p \leq n$ (лема 3).
\end{itemize}
Отже, кожний множник $p$ задовольняє умову $p \leq \dfrac{2n}{3}$.\\
Нехай $p > \sqrt{2n}$, тоді число $C_{2n}^n$ матиме до одного множник $p$. За лемою 2, $p^{R(p,n)} \leq 2n$, також $\pi(x) \leq x-1$ (бо $1$ ані просте, ані складене). Використовуючи лему 1 та лему 4, отримаємо:\\
$\dfrac{4^n}{2n} \leq C_{2n}^n \overset{\text{основна Th.\ арифметики}}{=} \displaystyle\left( \prod_{p \leq \sqrt{2n}} p^{R(p,n)} \right) \left( \prod_{\sqrt{2n} < p \leq \frac{2n}{3}} p^{R(p,n)} \right) < \\ < \left( \prod_{p \leq \sqrt{2n}} 2n \right) \left( \prod_{p \leq \frac{2n}{3}} p \right) \leq (2n)^{\pi(\sqrt{2n})} 4^{\frac{2n}{3}} \leq (2n)^{\sqrt{2n}-1} 4^{\frac{2n}{3}}$.\\
Отже, $4^{\frac{n}{3}} \leq (2n)^{\sqrt{2n}}$, тобто звідси $2^{\sqrt{2n}} \leq (2n)^3$. Тобто $\sqrt{2n} \leq 3 \log_2 (2n)$.\\
Зауважимо, що при $n = 426$ нерівність виконується, але вже не при $n = 427$. Оскільки $\log_2(2n)$ -- випукла, нерівність виконується на інтервалі.\\
Отже, ми отримали $n < 427$, що не можна припустити. Суперечність! 
\end{proof}
\newpage

\section{Prime number theorem}
\begin{theorem}
$\pi(x) \sim \dfrac{x}{\ln x}$ при $x \to +\infty$.
\end{theorem}

Дана теорема не є простою в додвенні. Будемо до цього рухатися поступово. Оскільки я ще не знаю комплексного аналізу, то цей розділ буде тимчасово недоповненим.

\begin{definition}
Визначимо \textbf{функцію Чебишова} таким чином:
\begin{align*}
\psi(x) = \sum_{p \leq x} \left[ \dfrac{\ln x}{\ln p} \right] \ln p, \quad x > 0
\end{align*}
\end{definition}

\begin{proposition}
$\pi(x) \sim \dfrac{x}{\ln x} \iff \psi(x) \sim x$ (обидва при $x \to +\infty$).\\
По суті кажучи, ми записали еквівалентність до prime number theorem.
\end{proposition}

\begin{proof}
\leftproof Дано: $\psi(x) \sim x$.\\
Спочатку зауважимо, що $\psi(x) \leq \displaystyle\sum_{p \leq x} \ln x = \pi(x) \ln x$. Звідси випливає, що $\dfrac{\psi(x)}{x} \leq \dfrac{\pi(x) \ln x}{x}$. Тоді, взявши $x \to +\infty$, ми отримаємо $1 \leq \displaystyle\liminf_{x \to +\infty} \dfrac{\pi(x) \ln x}{x}$.\\
Тепер зауважимо, що при $\alpha \in (0,1)$ маємо $\psi(x) \geq \displaystyle\sum_{p \leq x} \ln p \geq \sum_{x^\alpha < p \leq x} \ln p \geq (\pi(x) - \pi(x^\alpha)) \ln x^\alpha \geq \alpha(\pi(x) - x^\alpha) \ln x$. Звідси випливає, що $\dfrac{\psi(x)}{x} \geq \dfrac{\ln x}{x} \alpha (\pi(x) - x^\alpha)$. Тоді, взявши спочатку $x \to +\infty$, а згодом $\alpha \to 1-0$, отримаємо $1 \geq \displaystyle\limsup_{x \to +\infty} \dfrac{\pi(x) \ln x}{x}$.\\
Ці дві нерівності свідчать як раз про $\dfrac{\pi(x) \ln x}{x} \to 1$, тобто $\pi(x) \sim \dfrac{x}{\ln x}, \quad x \to +\infty$.
\bigskip \\
\rightproof Дано: $\pi(x) \sim \dfrac{x}{\ln x}$.\\
Уже доводили оцінку $\dfrac{\psi(x)}{x} \leq \dfrac{\pi(x) \ln x}{x}$ в напрямку \leftproof. Зворотна оцінка по-іншому отримується: (TODO:?)\\
$\psi(x) > \displaystyle\sum_{p \leq x} (\ln x - \ln p) = (\ln x - \ln p) \pi(x)$ 
\end{proof}
...
\begin{remark}
Зокрема якщо придивитись на множину $U_n$, то функція Ойлера перепишеться інакшим чином:
$$\varphi(n) = \# U_n$$
\end{remark}
...
\iffalse
А ось цей наслідок дозволяє дуже просто довести властивість мультиплікативності функції Ойлера, зокрема в силу ізоморфності, ми отримаємо:\\
...
І якщо пригадати, що таке функція Ойлера, ми отримаємо\\
$\varphi(mn) = \varphi(m)\varphi(n)$.\\
Тим самим ми простіше довели мультиплікативність функції Ойлера.
...
\bigskip \\
Тим самим, функцію Ойлера можна визначити дещо інакше:\\
...
Таким чином, доводиться $\varphi(m) = \varphi(m_1) \dots \varphi(m_r)$ - мультиплікативність функції Ойлера.
