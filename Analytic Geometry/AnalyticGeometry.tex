...
\begin{definition}
Два вектори $\vec{a},\vec{b}$, що лежать на паралельних прямих (\textit{або на одній прямій}), називають \textbf{колінеарними}.\\
\textit{Позначення}: $\vec{a} \parallel \vec{b}$.
\bigskip \\
...
\bigskip \\
\textit{Позначення}: $|\vec{a}|$ -- довжина вектора.
\end{definition}
...
Задамо ілюстративним чином операції над векторами:
	...
	\item \textbf{додавання;} (\textit{є два способи, як додавати});
\begin{figure}[H]
\centering
	\begin{tikzpicture}
...
\centering
	...
	\draw[thick, red, ->] (0,0)--(4,2) node at (2,0.5) {$\vec{a}+\vec{b}$};
	\end{tikzpicture}
	\qquadє два способи, як додавати
	\begin{tikzpicture}
...
\begin{definition}
	Вектори, що лежать на перпендикулярних прямих, називають \textbf{ортогональними}.\\
\textit{Позначення}: $\vec{a} \perp \vec{b}$.
\end{definition}
...
(\textit{іншими словами, вектори колінеарні тоді й лише тоді, коли один вектор можна масштабувати до іншого})
\end{proposition}

\begin{proof}
\rightproof Дано: $\vec{a} \parallel \vec{b}$. Тоді виникають два варіанти:\\
I. $\vec{a} \uparrow \uparrow \vec{b}$.\\
Тоді геометрично можна підібрати число $\lambda = \dfrac{|\vec{a}|}{|\vec{b}|}$, для якого $\vec{a} = \lambda \vec{b}$.\\
II. $\vec{a} \uparrow \downarrow \vec{b}$.\\
Тоді геометрично можна підібрати число $\lambda = -\dfrac{|\vec{a}|}{|\vec{b}|}$, для якого $\vec{a} = \lambda \vec{b}$.
\bigskip \\
\leftproof Дано: $\vec{a} = \lambda \vec{b}$ для деякого $\lambda \in \mathbb{R}$. Звідси ясно, що $\vec{a} \parallel \vec{b}$.
\end{proof}
...
Задані два вектори $\vec{a}$, $\vec{b}$, які неколінеарні. Тоді для кожного вектора $\vec{c}$ на площині існують єдині числа $\alpha, \beta \in \mathbb{R}$, для яких 
\begin{align*}
\vec{c} = \alpha \vec{a} + \beta \vec{b}
\end{align*}
(\textit{тобто третій вектор на площині завжди єдиним чином виражається через два неколінеарних вектори}).
\end{theorem}

\begin{proof}
	Маємо вектори $\vec{a}$, $\vec{b}$, $\vec{c}$ на площині. Ми перемістимо їх, щоб лежали на одному спільному початку.
\begin{figure}[H]
\centering
	\begin{tikzpicture}
...
І. \textit{Існування.}\\
	...
	Також із малюнку зауважимо, що $\overrightarrow{AD} \parallel \vec{b}$, звідси $\overrightarrow{AD} = \beta \vec{b}$ для деякого $\beta \in \mathbb{R}$. Аналогічно $\overrightarrow{AB} \parallel \vec{a}$, звідси $\overrightarrow{AB} = \alpha \vec{a}$ для деякого $\alpha \in \mathbb{R}$.\\
	Отже, $\vec{c} = \alpha \vec{a} + \beta \vec{b}$.
\bigskip \\
II. \textit{Єдиність.}\\
!Припустимо, що розклад не є єдиним, тобто існують числа $\alpha', \beta'$, для яких $\vec{c} = \alpha' \vec{a} + \beta' \vec{b}$. Тоді 
\[
\vec{0} = \vec{c} - \vec{c} = (\alpha-\alpha') \vec{a} + (\beta - \beta') \vec{b} \implies (\alpha-\alpha')\vec{a} = (\beta' - \beta)\vec{b} \overset{\textrm{\prpref{collinear_vectors}}}{\implies} \vec{a} \parallel \vec{b}.
\]
...
\begin{remark}
Уже довели, що кожний вектор $\vec{c}$ виражається через базисні вектори як $\vec{c} = \alpha \vec{a} + \beta \vec{b}$. Одержали розклад за базиом. Коефіцієнти $\alpha, \beta$ задають \textbf{координати} вектора $\vec{c}$ в базисі $\vec{a},\vec{b}$.
\end{remark}
