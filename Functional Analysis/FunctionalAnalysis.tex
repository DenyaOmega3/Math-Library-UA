...
\begin{example}
Покажемо, що оператор $A \colon C^1([0,1]) \to C([0,1])$ буде необмеженим:
\[
(Af)(t) = f'(t).
\]
...
\begin{definition}
Бієктивний оператор $A \in \mathcal{B}(X,Y)$ називається \textbf{оборотно-обмеженим}\footnote{У книжках цей оператор називають просто \textbf{оборотним}, але для мене це призводить до плутанини зі стандартним означенням оборотного оператора (\textit{як бієктивне відображення}).}, якщо
\begin{align*}
...
\begin{theorem}
Маємо $A \in \mathcal{B}(X,Y)$ --- сюр'єктивний\footnote{Цю теорему можна дещо узагальнити. Зокрема замість сюр'єктивності оператора $A$ (\textit{тобто $Y = \Im A$}) можна лише попросити, щоб $\Im A$ була скрізь щільною (\textit{тобто $Y = \overline{\Im A}$})}. Тоді
\begin{center}
$A$ бієктивний та оборотно-обмежений $\iff \exists m > 0: \forall x \in X: \|Ax\| \geq m\|x\|$.
\end{center}
\end{theorem}

\begin{proof}
\rightproof Дано: $A^{-1} \in \mathcal{B}(Y,X)$, тобто $\|A^{-1}y\| \leq \|A^{-1}\| \|y\|$ для кожного $y \in Y$. Оскільки $A$ --- бієктивний оператор, то існує єдиний $x \in X$, для якого $y = Ax$. Значить, підставивши в нерівність, отримаємо $\|A^{-1} Ax\| = \|x\| \leq \|A^{-1}\| \|Ax\|$. Якщо покласти $m = \|A^{-1}\|^{-1} > 0$, то отримаємо бажану нерівність $\|Ax\| \geq m\|x\|$.
\bigskip \\
...
\begin{proof}
Доведемо, що $A^{-1}$ --- неперервне відображення (\textit{із топологічного означення}). Справді, якщо $U$ --- відкрита в $X$, то його прообраз $(A^{-1})^{-1}(U) = A(U)$ --- відкрита в $Y$ (\textit{наш оператор $A$ задовольняє теорему про відкрите відображення}). Власне, тоді $A^{-1} \in \mathcal{B}(Y,X)$.
\end{proof}
\noindent
...
\centering
\begin{tikzcd}
& \Gamma(A) \arrow[dl,swap,"\pi_1"] \arrow[dr,"\pi_2"] & \\
X_1 \arrow[rr, "A"] & & X_2
\end{tikzcd}
\end{figure}
...
\end{multline*}
За умовою $q < 1$, тому фундаментальність доводиться миттєво. Оскільки простір $X$ банахів, то звідси $\mathcal{B}(X)$ теж банахів, тому $S_n \substack{\to \\ \to} S \in \mathcal{B}(X)$.\\ Зрозуміло, що $I-A \in \mathcal{B}(X)$. Залишилося довести, що $I-A$ є оборотним оператором до $S$, тобто 
\[
(I-A)S = S(I-A) = I.
\]
...
Нехай $X,Y$ --- банахові простори та оператори $A,B \in \mathcal{B}(X,Y)$, причому оператор $A$ --- оборотно-обмежений та $B$ задовольняє нерівність $\|B\| < \|A^{-1}\|^{-1}$. Тоді оператор $A+B$ буде оборотно-обмеженим.
\end{theorem}

\begin{proof}
Спочатку зауважимо, що виконується наступне:
\[
A + B = A(I + A^{-1}B).
\]
Уже $A$ є оборотно-обмеженим. Хочеться довести, що $I+A^{-1}B \colon X \to X$ буде також оборотно-обмеженим, тим самим $A+B$ стане оборотно-обмеженим. Для цього зауважимо, що
\[
\|A^{-1}B\| \leq \|A^{-1}\|\, \|B\| < 1,
\]
а далі маємо право використати попередню теорему.
\end{proof}

\subsection{Продовження неперервних операторів, теорема Гана-Банаха}
Нехай $X,Y$ --- нормовані простори, $X_0 \subset X$ та $A \in \mathcal{B}(X_0,Y)$. Головне питання: чи існує розширення $\tilde{A} \in \mathcal{B}(X,Y)$ оператора $A$? Причому нас буде цікавити таке розширення, щоб $\|\tilde{A}\| = \|A\|$.
...
\begin{proof}
Нехай є послідовність $(x_n)_{n \in \mathbb{N}} \subset X_0$, де $x_n \to x \in X$. Зауважимо, що $(Ax_n)_{n \in \mathbb{N}} \subset Y$ --- фундаментальна. У силу банаховості $Y$ матимемо збіжність $(Ax_n)_{n \in \mathbb{N}}$. Тож визначимо оператор 
\[
\tilde{A} \colon X \to Y, \qquad \tilde{A}x = \displaystyle\lim_{n \to \infty} Ax_n.
\]
\begin{enumerate}[wide=0pt,label={\Roman*.}]
\item \textit{$\tilde{A}$ визначений коректно}.\\
...
\]
\item \textit{$\tilde{A}$ розширює оператор $A$}.\\
...
\item \textit{$\tilde{A}$ лінійний оператор}.\\
...
\fi
\item $\|\tilde{A}\| = \|A\|$.\\
Для кожної точки $x \in X$ оберемо послідовність $(x_n)_{n \in \mathbb{N}} \subset X_0$ таку, що $x_n \to x$. Оскільки $A$ --- обмежений, то $\|Ax_n\| \leq \|A\| \|x_n\|$. Спрямовуючи $n \to \infty$, ми отримаємо $\|\tilde{A}x\| \le \|A\| \|x\|$. Автоматично довели, що $\tilde{A}$ --- обмежений оператор. Раз це виконується для всіх $x \in X$, то одержимо 
\[
\|\tilde{A}\|= \displaystyle\sup_{\|x\| = 1} \|\tilde{A}x\| \leq \sup_{\|x\| = 1} \|A\| \|x\| = \|A\|.
\]
Тобто звідси $\|\tilde{A}\| \leq \|A\|$. Зважаючи на зауваження вище, маємо $\|\tilde{A}\| = \|A\|$.

\item \textit{$\tilde{A}$ --- єдине розширення}.\\
...
\] 
Отримали суперечність! Коротше, у нас тільки єдиний такий оператор. \qedhere
\end{enumerate}
\end{proof}
\noindent
Існує не зовсім неперервний варіант розширитися, який можливо для лінійних неперервних функціоналів. Загострювати увагу будемо саме на них.
\bigskip \\
\textit{Позначення}: $X' = \mathcal{B}(X,\mathbb{R})$ --- множина всіх лінійних неперервних функціоналів на $X$.
...
\textit{Доведення громізке, для охочих можете подивитися в окремому pdf файлі.}
\end{theorem}
...
\section*{Списки тверджень, які я ще не доводив}
\begin{itemize}[wide=0pt]
\item Нехай $A \in \mathcal{B}(X,Y)$. Тоді
\begin{center}
$A$ --- бієктивний, оборотно-обмежений $\iff \exists m > 0: \forall x \in X: \|Ax\| \geq m \|x\|$ та $\Im A$ --- щільна в $Y$.
\end{center}
(\textit{довів, але закоментував})

\iffalse
\rightproof не змінюється.\\
\leftproof Тут одержимо, що $A$ все одно ін'єктивне, але тоді $A \colon X \to \Im A$ має обротно-обмежений оператор $C \colon \Im A \to X$. Нехай тепер $y \in Y \setminus \Im A$, тоді в силу щільності існує послідовність $(y_n)_{n \in \mathbb{N}} \subset \Im A$ така, що $y_n \to y$, де кожен $y_n = Ax_n$. Оскільки $C$ є неперервним, то $(Cy_n)_{n \in \mathbb{N}} = (x_n)_{n \in \mathbb{N}}$ збіжний та $Cy_n \to Cy = x$. Оскільки $A$ --- неперервний, то $Ax_n$ також буде збіжним до $Ax$, причому $y = Ax$.\\
Коротше, для кожного $y$ знайшли $x = \displaystyle\lim_{n \to \infty} Cy_n$, для якого $y = Ax$.
\fi
\end{itemize}
\end{document}
