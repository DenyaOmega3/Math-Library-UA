...
Позначення: $\mathcal{B}(X,Y)$ --- множина всіх обмежених операторів $X \to Y$.
\end{definition}

\begin{remark}
Нехай $A \in \mathcal{B}(X,Y)$. Зауважимо, що множина всіх констант, які обмежують оператор, тобто множина 
\[
...
\]
Таким чином, $\|A\|-\varepsilon$ --- це константа, яка обмежує оператор, тоді за означенням норми, $\|A\| - \varepsilon \geq \|A\|$, а це --- суперечність!
\end{proof}
...
\noindent
Повернемось до питання, чому в лінійній алгебрі (де переважно досліджувалися оператори між скінченновимірними просторами) ми не розглядали обмежені оператори. Бо для таких випадків уже все відомо.

\begin{proposition}
Нехай $X,Y$ --- нормовані простори, $\dim X < \infty$ та $A \in \mathcal{L}(X,Y)$. Тоді $A \in \mathcal{B}(X,Y)$.\\
...
Простори $\mathcal{B}(\mathbb{R},F)$ та $F$ задають природну ізометрію. Тому надалі
\begin{align*}
\mathcal{B}(\mathbb{R},F) = F.
\end{align*}
\end{proposition}

\begin{proof}
\rightproof Кожному елементу $y \in F$ поставимо в відповідність відображення $\mathbb{R} \to F$, яке працює як $\lambda \mapsto \lambda y$ та яке є обмеженим, оскільки $\| \lambda y\| = \|y\| |\lambda|$. Отже, побудували відображення
\begin{align*}
\varphi \colon F \to \mathcal{B}(\mathbb{R},F), \qquad \varphi(y) = (\lambda \mapsto \lambda y).
\end{align*}
Думаю, неважко переконатися, що це лінійне відображення.
\bigskip \\
\leftproof Кожному елементу $f \in \mathcal{B}(\mathbb{R},F)$ поставимо в відповідність елемент $f(1) \in F$. Отже, побудували відображення
\begin{align*}
\psi \colon \mathcal{B}(\mathbb{R},F) \to F,\qquad \psi(f) = f(1).
\end{align*}
...
\begin{align*}
X = \bigcup_{n =1}^\infty \overline{A(B(0;n))}.
\end{align*}
...
Доведемо, що $B(0;r) \subset \overline{A(B(0;1))}$. Нехай $v \in B(0;r)$, тоді $2v + v_1 \in B(v_1;2r)$. Оскільки $v_1 \in A(B(0;1))$, то звідси $u_1 = Av_1$ для деякого $u_1 \in B(0;1)$. Точку $2v$ запишемо в такому вигляді:
\[
...
Оберемо точку $b_2 \in A(U)$, тобто $b_2 = Ab_1$ для деякого $b_1 \in U$. За відкритістю знайдеться куля $B(b_1;\varepsilon) \subset U$. За лемою $A(B_1(0;1)) \supset B(0;\delta)$ для деякої кулі. Звідси
\[
B(b_2, \varepsilon \delta) = b_2 + \varepsilon B(0;\delta) \subset b_2 + \varepsilon A(B(0;1)) = A(b_1) + \varepsilon A(B(0;1)) = A(b_1 + B(0;\varepsilon)) = T(B(b_1;\varepsilon)).
\]
...
Нехай $x$ такий, що $\|x\| = 1$. Тоді цілком ясно, що $\dfrac{\delta_0}{2}x \in B(0;\delta_0)$, що дає оцінку $\displaystyle \left\| A_n \left( \dfrac{\delta_0}{2} x\right) \right\| \leq 2k$ для кожного $n \in \mathbb{N}$. Іншими словами, $\| A_n x \| \leq \dfrac{4k}{\delta_0}$. Оскільки це виконується для всіх $x$, де $\|x\| = 1$, то звідси $\|T_n\| \leq \dfrac{4k}{\delta_0}$ для всіх $n \in \mathbb{N}$.
\end{proof}

%TODO обдумати
\iffalse
\begin{remark}
У теоремі Банаха-Штайнгауза умова того, що $X$ --- банахів, --- суттєва.
\end{remark}

\begin{example}
Зокрема розглянемо простір $c_0$ --- послідовності, що збігаються до нуля. Далі розглянемо небанахів підпростір $c_{00} \subset c_0$ --- послідовності, де всі члени нулі, починаючи з деякого номера.\\
Задамо послідовність операторів $l_n \colon c_{00} \to c_{00}$, що задається як
\[
l_n(x) = (x_1, 2x_2, \dots, n x_n, 0, 0, \dots).
\]
Зауважимо, що $l_n \in \mathcal{B}(c_{00})$, оскільки
\[
\|l_n(x)\| = \| (x_1,2x_2,\dots,n x_n, 0, 0,\dots) \| = \sqrt{x_1^2 + 2x_2^2 + \dots + n x_n^2} \leq n \| x \|.
\]
Крім того, для кожного $x \in c_{00}$ послідовність $(l_n(x))$ -- обмежена
\end{example}
\fi
...
\end{proof}
\noindent
Ще один гучний результат, який мав би бути минулого розділу потому, однак щойно доведена теорема буде потрібна на допомогу.
