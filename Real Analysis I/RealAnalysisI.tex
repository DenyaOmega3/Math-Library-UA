...
%\iffalse
	...
	А тут вже ми змогли визначити операцію $\cdot$, і цього теж мало. (TODO: залишити посилання для цього). Насправді, всі ці множини та операції $+,\ \cdot$ можна спробувати дуже строго формалізувати, проте цього робити не планую. Це не сильно вплине на якість вивчення матана.
	...
	\end{enumerate}
	Конструкцією $\mathbb{R}$ займатися не хочу на самому початку, просто тому що це нудно та затратно. Ми підемо іншим більш стандартним шляхом, який не призводить до зміни теорії.
...
Розглянемо таку картину -- коло Рімана. Нижній дотик кола буде відповідати точці $0$, а верхня точка -- точці $\infty$.
	...
	Отже, $[0,1]$ -- незліченна множина, а тому тим паче $\mathbb{R} \supset [0,1]$ -- незліченна множина.
	\end{proof}
...
%\fi
	...
	Існує ще ось таке позначення послідовності: $\{a_n\}_{n=1}^\infty$.
	\end{definition}
	
	\begin{example}
	Розглянемо декілька прикладів, де послідовності по-різному задаються:
	\begin{enumerate}[nosep,wide=0pt,label={\arabic*)}]
	\item $\{a_n, n \geq 1\} = \{\underset{a_1}{1},\underset{a_2}{2},\underset{a_3}{-1},\underset{a_4}{3},\underset{a_5}{0},\dots\}$ -- якась рандомна послідовність;
	\item $\{b_n, n \geq 1\}$, де $b_n = \dfrac{1}{n}$ -- послідовність, що задається формулою;
	\item $\{c_n, n \geq 1\}$, де $c_1 = 1$, а також $c_{n+1} = \dfrac{c_n}{n+1}$ -- рекурентна послідовність.
	\end{enumerate}
	\end{example}
	
	\begin{example}
	Така штука $a_n = \dfrac{1}{n - \the\year}$ не задаватиме числову послідовність, допоки ми не визначимо, чому дорівнює $a_{\the\year}$.
	\end{example}
	...
	У такому разі послідовність $\{a_n,n \geq 1\}$ називається \textbf{збіжною}. Якщо границі не існує, то тоді послідовність називається \textbf{розбіжною}.
	\end{definition}
	...
	\begin{proof}
	!Припустімо, що задано збіжну числову послідовність $\{a_n, n \geq 1\}$, для якої існують дві границі: $\displaystyle \lim_{n \to \infty} a_n = a', \ \lim_{n \to \infty} a_n = a''$. Ми вважатимемо, що $a_1 < a_2$, оскільки для $a_1 > a_2$ міркування є аналогічними.\\
	Оберемо число $\displaystyle \varepsilon = \frac{a''-a'}{3} > 0$. Тоді за означенням існують числа $N_1,N_2$ такі, що\\
	$\displaystyle\forall n \geq N_1: |a_n-a'|< \frac{a''-a'}{3} \implies a_n < a' + \frac{a''-a'}{3}$;\\
	$\displaystyle\forall n \geq N_2: |a_n-a''|< \frac{a''-a'}{3} \implies a_n > a'' - \frac{a''-a'}{3}$.\\
	Аби обидві нерівності працювали одночасно, ми зафіксуємо новий $N= \max\{N_1,N_2\}$. Тоді\\
	$\displaystyle \forall n \geq N: a_n < \frac{a'+(a'+a'')}{3} < \frac{a''+(a'+a'')}{3}<a_n$. \\
	...
	\label{one_over_n_converges_to_zero}
	Доведемо за означенням, що $\displaystyle\lim_{n \to \infty} \frac{1}{n} = 0$.\\
	Задамо довільне $\varepsilon > 0$. Необхідно знайти таке $N$, щоб $\displaystyle  \forall n \geq N: \left|\frac{1}{n}-0 \right|<\varepsilon$.\\
	$\displaystyle \abs{\frac{1}{n} - 0} < \varepsilon \iff \displaystyle \frac{1}{n}<\varepsilon \iff n > \frac{1}{\varepsilon}$.\\
	Зафіксуємо $\displaystyle N = \left[\frac{1}{\varepsilon} \right] + 1$. Тоді маємо: $\forall \varepsilon > 0: \exists N = \displaystyle \left[\frac{1}{\varepsilon} \right] + 1: \forall n \geq N: n > \frac{1}{\varepsilon} \implies \abs{\frac{1}{n} - 0} < \varepsilon$.\\
...
\end{figure}
	...
	Задамо довільне $E>0$. Необхідно знайти $N$, для якого $\forall n \geq N: 2^n>E$.\\
	Раніше доводили, що $2^n \geq n$. Вимагатимемо тепер, щоб $n > E$.\\
	Фіксуємо $N=\left[ E \right] + 2$. Тоді $\forall n \geq N: n > E$, а тим паче $2^n > n > E$.\\
...
Задамо довільне $E > 0$. Необхідно знайти $N$, для якого $\forall n \geq N: |(-1)^n 2^n| = 2^n > E$. Але це ми вже доводили зверху. Важливо тут те, що не можна визначитись, чи $+\infty$, чи $-\infty$ через знакочередованість.
\end{example}
	
	\subsection{Обмежені послідовності}
	\begin{definition}
	...
	\begin{proof}
	Нехай задана збіжна послідовність $\{a_n, n \geq 1\}$, тобто для неї існує границя $\displaystyle \lim_{n \to \infty} a_n = a$. Зокрема для $\varepsilon = 1$ існуватиме $N$, для якого $\forall n \geq N: \abs{a_n-a}<1$.\\
	Спробуємо оцінити вираз $|a_n|$ для нашого бажаного:\\
	$|a_n| = |a_n - a + a| \leq |a_n-a|+|a| < 1 + |a|$ -- це виконується $\forall n \geq N$. Тобто обмежені всі члени, починаючи з $N$.\\
	...
	\subsection{Нескінченно малі/великі послідовності, арифметика границь}
	\begin{definition}
	...
	\begin{definition}
	Послідовність $\{a_n, n \geq 1\}$ назвемо \textbf{віддаленою від нуля}, якщо
	\begin{align*}
	...
	Задані п'ять послідовності: $\{a_n\}, \ \{b_n\},\ \{c_n\},\ \{d_n\}, \{p_n\}$ -- відповідно н.м.,\ н.м.,\ обмежена,\ н.в.,\ віддалена від нуля. Тоді:\\
	\begin{tabular}{ll}
	...
	\begin{proof}
	1) Маємо $\{a_n\},\ \{b_n\}$ -- обидва н.м. Нехай $\varepsilon > 0$, тоді існують такі $N_1,N_2$, що \\
	$\forall n \geq N_1: |a_n-0| = |a_n| < \dfrac{\varepsilon}{2} \qquad \forall n \geq N_2: |b_n-0| = |b_n| < \dfrac{\varepsilon}{2}$.\\
	Покладемо $N = \max\{N_1,N_2\}$. Тоді $|(a_n+b_n)-0| = |a_n+b_n| \leq |a_n|+|b_n| < \varepsilon$ для всіх $n \geq N$. Отже, $\{a_n+b_n\}$ -- н.м.
	\bigskip \\
	2) \textit{Буде доводитися десь аналогічно як 3), яке зараз доведемо.}
	\bigskip \\
	3) Маємо $\{a_n\}$ -- н.м. та $\{c_n\}$ -- обмежена. Нехай $\varepsilon > 0$. Тоді із н.м. існує $N$, для якого $\forall n \geq N: |a_n-0| = |a_n| < \dfrac{\varepsilon}{M}$. Число $M > 0$ ми взяли з означення обмеженості $\{c_n\}$, тобто в нашому випадку $|c_n| \leq M,\ \forall n \geq 1$. \\
	Отже, $\forall n \geq N: | (c_n \cdot a_n) - 0| = |c_n| \cdot |a_n| < M \dfrac{\varepsilon}{M} = \varepsilon$. Внаслідок чого $\{a_n \cdot c_n\}$ -- н.м.
	\bigskip \\
	4) Маємо $\{a_n\}$ -- н.м. Нехай $E > 0$. Взявши $\varepsilon = \dfrac{1}{E}$ та означення н.м., тоді існує таке $N$, що $\forall n \geq N: |a_n| < \dfrac{1}{E}$. Значить, $\left| \dfrac{1}{a_n} \right| > E$. Отже, $\left\{ \dfrac{1}{a_n} \right\}$ -- н.в.
	\bigskip \\
	5) \textit{Доводиться аналогічно як} 4)
	\bigskip \\
	6) \textit{Доводиться аналогічно як} 3)
	\end{proof}
	
	\begin{example}
	Розглянемо декілька прикладів:
	\begin{enumerate}[nosep,wide=0pt,label={\arabic*)}]
	\item $\displaystyle\lim_{n \to \infty} \dfrac{1}{2^n} = 0$, тому що $a_n = 2^n$ -- н.в., а тому $\dfrac{1}{a_n} = \dfrac{1}{2^n}$ -- н.м.\\
	\item $\displaystyle\lim_{n \to \infty} \dfrac{(-1)^n}{n} = 0$, тому що $a_n = (-1)^n$ -- обмежена та $b_n = \dfrac{1}{n}$ -- н.м.
	\end{enumerate}
	\end{example}
	
	\begin{theorem}[Про характеризацію збіжної послідовності]
	Задано послідовність $\{a_n, n \geq 1\}$.\\
	Послідовність $\{a_n, n \geq 1\}$ -- збіжна до $a$ $\iff$ існує н.м. послідовність $\{\alpha_n, n \geq 1\}$, де $a_n = a+\alpha_n$.
	\end{theorem}
	
	\begin{proof}
	\rightproof Дано: $\{a_n, n \geq 1\}$ -- збіжна до $a$, тобто $\forall \varepsilon > 0: \exists N: \forall n \geq N: |a_n-a| < \varepsilon$. Позначимо $a_n-a=\alpha_n$. Тоді $a_n=a+\alpha_n$ та послідовність $\{\alpha_n, n \geq 1\}$ -- н.м., оскільки $|\alpha_n - 0| = |\alpha_n| = |a_n - a| < \varepsilon$.
	\bigskip \\
	...
	\begin{proof}
	Обидві послідовності збіжні за умовою. Тоді за попередньою теоремою $a_n = a + \alpha_n$ та $b_n=b+\beta_n$, де $\{\alpha_n\}, \{\beta_n\}$ -- н.м. послідовності.\\
	...
	2) \textit{Зрозуміло}.
	\bigskip \\
	3) $a_n b_n - ab = (a+\alpha_n)(b+\beta_n) - ab = \alpha_n b + \alpha_n \beta_n + a \beta_n = \gamma_n$, причому послідовність $\{\gamma_n = \alpha_n b + \alpha_n \beta_n + a \beta_n \}$ -- н.м.  Отже, послідовність $\{a_n b_n, n \geq 1\}$ -- збіжна та має границю $\displaystyle \lim_{n \to \infty} (a_n \cdot b_n) = ab = \lim_{n \to \infty} a_n \cdot \lim_{n \to \infty} b_n$.
	\bigskip \\
	4) У принципі, це є наслідком 3), якщо представити послідовність $\dfrac{a_n}{b_n} = a_n \cdot \dfrac{1}{b_n}$. Треба лише довести, що $\dfrac{1}{b_n} \to \dfrac{1}{b}, n \to \infty$.\\
	...
	Таким чином, можна казати, що $\dfrac{1}{b_n} \to \dfrac{1}{b}, n \to \infty \implies \dfrac{a_n}{b_n} \to \dfrac{a}{b}$, тобто $\left\{ \dfrac{a_n}{b_n}, n \geq 1 \right\}$ -- збіжна.
	\end{proof}
	
	\begin{example}
	Знайти границю $\displaystyle \lim_{n \to \infty} \dfrac{(-1)^n + \dfrac{1}{n}}{\dfrac{1}{n^2}- (-1)^n}$.\\
	$\displaystyle \lim_{n \to \infty} \dfrac{(-1)^n + \frac{1}{n}}{\dfrac{1}{n^2}- (-1)^n} = \lim_{n \to \infty} \dfrac{1 + \dfrac{1}{(-1)^n n}}{\dfrac{1}{(-1)^n n^2} - 1} \boxed{=} \dfrac{\displaystyle\lim_{n \to \infty} \left( 1 + \dfrac{1}{(-1)^n n} \right)}{\displaystyle\lim_{n \to \infty} \left( \dfrac{1}{(-1)^n n^2} - 1 \right)} \boxed{\boxed{=}}
	...
	\begin{remark}
	Вище було правильне розв'язання. Як було б неправильно: \\
	$\displaystyle \lim_{n \to \infty} \dfrac{(-1)^n + \frac{1}{n}}{\dfrac{1}{n^2}- (-1)^n} = \dfrac{\displaystyle \lim_{n \to \infty} \left( (-1)^n + \frac{1}{n} \right) }{\displaystyle \lim_{n \to \infty} \left( \frac{1}{n^2} - (-1)^n \right)} = \dfrac{\displaystyle \lim_{n \to \infty} (-1)^n + \lim_{n \to \infty} \frac{1}{n} }{\displaystyle \lim_{n \to \infty} \frac{1}{n^2} - \lim_{n \to \infty} (-1)^n} = \dots$\\
	...
	!Припустимо $a>b$. Розглянемо $\displaystyle \varepsilon = \frac{a-b}{2}$. Тоді за означенням границі існують $N_1,N_2$, для яких\\
	$\forall n \geq N_1:|a_n-a|<\varepsilon \implies a_n>a-\varepsilon$\\
	$\forall n \geq N_2:|b_n-b|<\varepsilon \implies b_n<b+\varepsilon$.\\
	...
	\begin{remark}
	Для нерівності $\geq$ аналогічно два твердження. Також ці твердження спрацьовують для $<$ або $>$, проте нерівність з границями залишається нестрогою.\\
	...
	\begin{remark}
	Теорему про 3 послідовності часто жартома називаю \textit{теорему про двох поліцаїв}.
	\end{remark}
	
	\begin{proof}
	Маємо $a_n \to a,\ b_n \to a$ при $n \to \infty$. Нехай $\varepsilon > 0$. Тоді існують $N_1,N_2$, для яких\\
	$\forall n \geq N_1: |a_n-a| < \varepsilon \implies a_n > a - \varepsilon$;\\
	$\forall n \geq N_2: |b_n-a| < \varepsilon \implies b_n<a+\varepsilon$.\\
	Покладемо $N=\max\{N_1, N_2, N'\}$ Номер $N'$ за умовою такий, що $\forall n \geq N': a_n \leq c_n \leq b_n$. Тоді \\
	$\forall n \geq N: a-\varepsilon< a_n \leq c_n \leq b_n < a+\varepsilon \implies |c_n - a|<\varepsilon$.\\
	Отже, $\displaystyle \lim_{n \to \infty} c_n = a$.
	\end{proof}
	
	\begin{example}[Важливий]
	Знайти границю $\displaystyle\lim_{n \to \infty} \sqrt[n]{a}$, де число $a>0$. \\
	...
	\begin{remark}
	Всі ці теореми спрацьовують для випадків, коли границі є нескінченностями.
	\end{remark}
	...
	\begin{tabular}{ll}
	\textbf{строго зростаючою}, якщо & $\forall n \geq 1: a_{n+1} > a_n$; \\
	\textbf{не спадною}, якщо & $\forall n \geq 1: a_{n+1} \geq a_n$; \\
	\textbf{строго спадною}, якщо & $\forall n \geq 1: a_{n+1} < a_n$; \\
	\textbf{не зростаючою}, якщо & $\forall n \geq 1: a_{n+1} \leq a_n$.
	\end{tabular}
	\end{definition}
	
	\begin{example}
	Дослідимо послідовність $\{a_n = \sqrt{n}, n \geq 1 \}$ на монотонність.\\
	$\displaystyle a_{n+1} - a_n = \sqrt{n+1} - \sqrt{n} = \frac{n+1-n}{\sqrt{n+1} + \sqrt{n}} = \frac{1}{\sqrt{n+1} + \sqrt{n}} > 0 \implies a_{n+1}>a_n$, тобто дана послідовність зростає.
	\end{example}
	
	\begin{theorem}[Теорема Ваєрштраса]
	Будь-яка обмежена зверху та неспадна, починаючи з деякого номеру, послідовність є збіжною.\\
	(або обмежена знизу та незростаюча, починаючи з деякого номеру).
	\end{theorem}

	\begin{proof}
	Нехай задана послідовність $\{a_n, n \geq 1\}$, яка є обмеженю зверху та неспадною. Оскільки вона монотонна, а ще -- обмежена, то $\exists \displaystyle\sup_{n \geq 1}\{a_n\} = a < +\infty$.\\
	...
	\end{gathered}$.\\
	Отримаємо такий ланцюг нерівностей: $\forall n \geq N: a-\varepsilon < a_N \leq a_n \leq a < a + \varepsilon \implies |a_n-a|<\varepsilon$.\\
	Отже, $\displaystyle \exists \lim_{n \to \infty} a_n = \sup_{n \geq 1}\{a_n\}$.
	\bigskip \\
	Для інших випадків монотонності (у тому числі строгої монотонності) все аналогічно.
	\end{proof}
	...
	\begin{remark}
	Такими самими міркуваннями можна довести, що $\dfrac{n^k}{b^n}$ при $b > 1$, $\dfrac{b^n}{n!}$ при $b > 0$, $\dfrac{n!}{n^n}$ -- всі вони будуть нескінченно малими, якщо $n \to \infty$.
	\end{remark}
	...
	Коротше, $\displaystyle \frac{a_{n+1}}{a_n} \geq 1 \implies a_{n+1} \geq a_n$. Тобто наша послідовність дійсно монотонно зростає.
	\bigskip \\
	...
	До речі, у $\left\{b_n = \left( 1 + \dfrac{1}{n} \right)^{n+1}, n \geq 1 \right\}$ така сама границя, оскільки виконується така рівність: \\ $\displaystyle\lim_{n \to \infty} \left( 1 + \dfrac{1}{n} \right)^{n+1} = \lim_{n \to \infty} \left( 1 + \dfrac{1}{n} \right)^n \lim_{n \to \infty} \left(1 + \dfrac{1}{n} \right) = e \cdot 1 = e$.
	\bigskip \\	
	...
	\begin{corollary}
	Будь-яка послідонвість натуральних чисел $\{n_k, k \geq 1\}$ -- додатно н.в.\\
	...
	\begin{proof}
	Нехай $\varepsilon > 0$. Тоді із $\displaystyle\lim_{n \to \infty} a_n = a$ випливає, що існує $N$, для якого $\forall n \geq N: |a_n-a| < \varepsilon$.\\
	...
	\bigskip \\
	\textit{I. Кількість значень послідовності -- скінченна} (як, наприклад, в \exref{example_of_divergent_sequence}). \\
	...
	\bigskip \\
	\textit{II. Кількість значень послідовності -- нескінченна} (як, наприклад, в \exref{one_over_n_converges_to_zero}). \\
	...
	Нагадаю, що трохи вище було позначення: $X$ -- множина часткових границь.
	...
	\end{align*}
	Часто ще $\displaystyle\varlimsup_{n \to \infty} a_n$ позначають за $\displaystyle\limsup_{n \to \infty} a_n$; \qquad $\displaystyle\varliminf_{n \to \infty} a_n$ позначають за $\displaystyle\liminf_{n \to \infty} a_n$.
	\end{definition}
...
\begin{proof}
	...
	\begin{proof}
	Маємо $\{a_n, n \geq 1\}$ -- фундаментальна, тож для $\varepsilon = 1$ існує $N$, для якого $\forall n,m \geq N: |a_n - a_m| < 1$. Схожимо чином отримуємо $|a_n| = |a_n - a_N + a_N| \leq |a_n - a_N| + |a_N| < 1 + |a_N|$ для всіх $n \geq N$.\\
	...
	\begin{proof}
	\rightproof Дано: $\{a_n,n \geq 1\}$ -- збіжна. Тоді якщо $\varepsilon > 0$, то існує номер $N$, для якого \\
	...
	Покладемо $N_* = \max\{N,K\}$. Тоді $\forall n \geq N_*$ маємо наступне: \\
	$|a_n - a| = |a_n - a_{n_{N_*}} + a_{n_{N_*}} - a| \leq |a_n - a_{n_{N_*}}| + |a_{n_{N_*}} - a| < \varepsilon$.\\
	Тобто $\displaystyle \exists \lim_{n \to \infty} a_n = a$.
	\end{proof}
	
	\begin{example}
	Розглянемо послідовність $\{a_n, n \geq 1\}$, де $a_n = \dfrac{\sin 1}{1^2} + \dfrac{\sin 2}{2^2} + \dots + \dfrac{\sin n}{n^2}$. Довести збіжність даної послідовності.\\
	...
	Установимо $N = \left[ \dfrac{1}{\varepsilon} \right] + 1$. Тоді $\forall n \geq N: \forall p \geq 1: |x_{n+p} - x_n| < \varepsilon$.\\
	...
	\begin{enumerate}[nosep,wide=0pt,label={\arabic*)}]
	\item $\{b_n\}$ -- н.в. та монотонно строго зростає (не обов'язково всюди, може з якогось номера);
	...
	Отже, маємо $\displaystyle \lim_{n \to \infty} \dfrac{a_{n+1} - a_n}{b_{n+1} - b_n} = L$. За означенням для $\varepsilon > 0$ існує $N_1$, для якого $\forall n \geq N_1: \abs{\dfrac{a_{n+1}-a_n}{b_{n+1}-b_n} - L} < \varepsilon \implies L - \varepsilon < \dfrac{a_{n+1} - a_n}{b_{n+1} - b_n} < L + \varepsilon$.\\
	Оскільки $\{b_n\}$ -- н.м., то для того ж $\varepsilon$ існує $N_2$, де $\forall n \geq N_2: b_n > 0$.\\
	...
	Зафіксуємо $k > N$ та просумуємо це до $k$. Після винесення дужок $(L-\varepsilon), (L+\varepsilon)$ маємо: \\
	$(b_{N+1} - b_N) + (b_{N+2} - b_{N+1}) + \dots + (b_{k+1} - b_{k}) = b_{k+1} - b_N$.\\
	Аналогічно $(a_{N+1} - a_N) + (a_{N+2} - a_{N+1}) + \dots + (a_{k+1} - a_{k}) = a_{k+1} - a_N$.\\
	Разом отримуємо оцінку $(L - \varepsilon)(b_{k+1} - b_N) < a_{k+1} - a_N < (L + \varepsilon)(b_{k+1} - b_N)$.\\
	Поділимо на $b_{k+1} > 0$ -- буде така оцінка:\\
	...
	$(L - \varepsilon) \left(1 - \dfrac{b_N}{b_{k+1}} \right) + \dfrac{a_N}{b_{k+1}}  < \dfrac{a_{k+1}}{b_{k+1}} < (L + \varepsilon) \left(1 - \dfrac{b_N}{b_{k+1}} \right) + \dfrac{a_N}{b_{k+1}}$.\\
	Спрямуємо $k \to \infty$, тоді за теоремою про нерівність та з урахуванням тим, що $\{b_n\}$ -- н.в., маємо:\\
	$(L-\varepsilon) \leq \displaystyle \lim_{k \to \infty} \dfrac{a_{k+1}}{b_{k+1}} \leq (L + \varepsilon) \implies \abs{\displaystyle \lim_{k \to \infty} \dfrac{a_{k+1}}{b_{k+1}} - L} \leq \varepsilon$, причому $\forall \varepsilon > 0$.\\
	Остаточно отримаємо: $\displaystyle \lim_{k \to \infty} \dfrac{a_{k+1}}{b_{k+1}} = L$.
	\bigskip \\
	II. \textit{Випадок при $L = +\infty$}. \\
	Тепер маємо $\displaystyle \lim_{n \to \infty} \dfrac{a_{n+1} - a_n}{b_{n+1} - b_n} = + \infty$, а тому для $E = 1$: існує $N$, для якого $\forall n \geq N: a_{n+1} - a_n > b_{n+1} - b_n \implies a_{n+1} > a_n$. Зафіксуємо $k > N$ та просумуємо нерівність $a_{n+1} - a_n > b_{n+1} - b_n$ до $k$.\\
	$\implies a_{k+1} - a_N > b_{k+1} - b_N \implies a_{k+1} > a_N-b_N + b_{k+1} \implies \displaystyle\lim_{k \to \infty} a_k = +\infty$.\\
	Отже, маємо $\{a_n\}$ -- н.в. та монотонно строго зростаюча.\\
	Також $\displaystyle \lim_{n \to \infty} \dfrac{b_{n+1} - b_n}{a_{n+1} - a_n} = 0$.
	Звідси $\displaystyle \lim_{n \to \infty} \dfrac{b_n}{a_n} = 0 \implies \lim_{n \to \infty} \dfrac{a_n}{b_n} = + \infty$.
	\bigskip \\
